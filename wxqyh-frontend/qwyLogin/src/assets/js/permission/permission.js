...
function getTrustApp(store, agentCode){
    return new Promise((resolve, reject) => {
        ...
        let url = isTrue ? `${baseURL}/application/application!ajaxIsTrustAgent.action` : `${baseURL}/application/application!ajaxIsTrustAgentList.action`;
        ...
        store.dispatch('header/setTrustApp', obj)
        .then(res => {
            resolve(res);
        })
        .catch(err => {
...
function getPermission(obj){
  ...
  if(trustAppData.length > 0){
    ...
    store.dispatch('header/setPermission')
    .then(res => {
      getTrustApp(store,'activity,ask,addressBook,car,checkwork,crm,contract,' +
