<template>
  <div class="qwui-check_header" :class="{'active':pageData&&pageData.length==0}">
    ...
    <!--  一键已阅按钮  -->
    <div class="qwui-one_key_read" v-if="footernav == '1' && pageData && pageData.length > 0" @click="oneKeyRead">
      全部标记为已阅
    </div>
...
import {oneKeyReadDiary} from '../../api/getData'
  ...
  methods:{
    oneKeyRead(){ //点击一键已阅
      var buttonConfig = {
        primaryBtn:{name:"确定", callBack:this.doOneKeyRead},
        defaultBtn:{name:"取消", callBack:null},
      };
      _.alert('标记为已阅',"确认将所有未阅标记为已阅？",buttonConfig)
    },
    doOneKeyRead(){
      oneKeyReadDiary((res)=>{
...
.diaryList .qwui-list_item_Important{
  ...
  .qwui-check_header:after, .qwui-check_box_list:after, .qwui-one_key_read:after{
    content: " ";
    position: absolute;
    left: 0;
    bottom: 0;
