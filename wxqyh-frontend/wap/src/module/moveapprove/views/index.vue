<template>
  <div class="wrap" v-windowscroll="scrollFun">
    ...
    <div v-show="showSearchBar" class="qwui-menu_tab_fixed active">
      <search_bar :searchinput="searchBarData" :keyWord="searchBarData.keyWord" @listenToSearchBox="searchKeywordClick" ></search_bar>
    </div>
    ...
    <nav-group
      ...
      @change="tabOneClickMy"
    >
      <nav-item
        icon="iconfont icon-renwufenpai-daichuli"
        :info="Number(home.totalRows)"
      >待处理</nav-item>
    ...
    <Mask_fix :show.sync="showMaskFix" :colorType="''" :modal="true" :modalExtent="modalExtent" @modelClick="closeClick">
      <div class="move_tolank_box" v-show="showCloseIcon">
        <a class="move_tolank" id="moveForm" target="_blank" href="http://d.do1.com.cn/BSHS"
           @click="baiduTraveEvent">使用表单流程，上千种表单模板随你挑</a>
        <i class="qwui-icon_arrow_fr icon_orange"></i>
      </div>
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      this.tabActiveIndex =  this.$route.name=="home"? 0 : 1;
      //获取当前搜索框缓存的关键字
      let cacheSearchValue = window.sessionStorage.getItem("moveapprove_" + this.$route.name);
      if (cacheSearchValue && cacheSearchValue != "") {
        this[this.$route.name].keyWord = cacheSearchValue;
        this.searchBarData.keyWord = cacheSearchValue;
      }
      //初始化请求
...
//            self.home.totalRows = result.data.todoNum? result.data.todoNum : 0;
      ...
      searchKeywordClick(data){
        ...
        this.searchBarData.keyWord = data;
        window.sessionStorage.setItem("moveapprove_" + this.$route.name, data);
        //重新请求列表
        this.requestGetdata(this.settingOptions(this.$route.name));
      },
      //接口数据处理
      ...
      baiduTraveEvent(){
        _hmt.push(['_trackEvent','moveapprove', 'click', 'literature']);
      },
      scrollFun(){
...
<style scoped>
  ...
  .move_tolank_box .qwui-icon_arrow_fr{
    right: 15px;
  }
  .qwui-icon_arrow_fr.icon_orange:after{
    color: #F8A421;
  }
  .footer-height{
    height: 49px;
