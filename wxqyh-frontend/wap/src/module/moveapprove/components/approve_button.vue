  ...
  <div>
    <flow_action_btn_group>
      <div style="display:flex; flex: 1">
        <flow_action_btn v-if="agreeBtn || agreeEndBtn || approveBtn || returnApproveBtn || bottomCreatorAction || showEndBtn" text="评论" icon="icon-shenpiqingshi-pinglun" @actionButtonClick="showChatClick"></flow_action_btn>
        <div v-if="!agreeBtn && !agreeEndBtn && !approveBtn && !returnApproveBtn && !bottomCreatorAction && !showEndBtn" class="comment_btn_add" @click="showChatClick">
          <input class="comment-chat_input" readonly="readonly" type="text" :value="value" :placeholder="chatPlaceholder"></div>
        ...
        <flow_action_btn v-if="showEndBtn" icon="icon-shenpiqingshi-guanbi" text="结束" @actionButtonClick="closeApprove"></flow_action_btn>
    ...
    <dialog_mask_bottom :dialogConfig="dialogBottomConfig">
      ...
      <dialog_mask_bottom_item slot="item" v-if="moreCreatorEnd" name="关闭" @dialogBtnClick="closeApprove"></dialog_mask_bottom_item>
...
<script>
  ...
  * */
  ...
  import {qwui_button, button_group,flow_action_btn,flow_action_btn_group} from '@/components/base/button'
  ...
  export default {
    ...
    computed: {
      ...
      // 更多那里显示 编辑
      moreCreatorEdit() {
        return this.creatorAction && (this.agreeEndBtn || this.returnApproveBtn || this.agreeBtn || this.showEndBtn)
      },
      // 更多那里显示 关闭
      moreCreatorEnd() {
        return this.creatorAction && (this.agreeEndBtn || this.returnApproveBtn || this.agreeBtn) && !this.showEndBtn
      ...
      returnApproveBtn() {
        ...
        if (!!this.detail && !!this.FlowAudit) {
          flag = (
            ...
            && this.FlowAudit && this.FlowAudit.audtiFlag !== '3'           // 下一节点有部门负责人
          )
        }
      ...
      showEndBtn() {
        if (!!this.detailData || !!this.FlowAudit) {
          return (
            this.detail.isover != -1                                                 // 未关闭
            && this.detailData.createLeave                                           // 提单人已离职
            || ( this.FlowAudit && this.FlowAudit.vo
                 && this.FlowAudit.vo.handleAccounts.indexOf(_.userId) > -1          // 是当前处理人
      ...
      moreApproveBtnShow() {
        ...
        || this.assignBtn             // 加签按钮
        || this.moreCreatorEnd        // 更多-关闭按钮
        || this.moreCreatorEdit     // 关闭、编辑按钮
...
//            this.$store.commit('updateAppointNodeVOList', result.appointNodeAndUserVOList);
      ...
      getFlowSerial() {
        ...
        let dataParam = {
          refId: this.detail.moveapproveId,
          isFree: this.detail.isFree == '0' ? 2 : this.detail.isFree//固定流程转为2
        }
