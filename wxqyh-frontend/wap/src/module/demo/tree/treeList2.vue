  ...
  export default {
    ...
    data() {
      return {
        ...
        CrmUsersIds: [],          // 保存已选择的客户ID，页面存在多种层级类型的选择时，才声明。只有一种类型时从 store 的selectValue获取
        PositionIds: [],          // 保存已选择的行业ID，页面存在多种层级类型的选择时，才声明。只有一种类型时从 store 的selectValue获取
        CrmUsersData: [],          // 保存已选择的客户，页面存在多种层级类型的选择时，才声明。只有一种类型时从 store 的selectValue获取
        PositionData: [],          // 保存已选择的行业，页面存在多种层级类型的选择时，才声明。只有一种类型时从 store 的selectValue获取
      }
    },
    computed:{
      ...mapState({
    ...
    methods: {
      ...
      confirmSelected() {
        ...
        if(this.selectTreeType === 'showCrmUsersData') {
          this.CrmUsersIds= this.selectValue
          this.CrmUsersData = this.currentSelected
        }
        if(this.selectTreeType === 'showPositionData') {
          this.PositionIds = this.selectValue
          this.PositionData = this.currentSelected
        }
      },
      // 查看已选择的数据
      showSelectedItem() {},
      ...
      showCrmUsersData() {
        ...
        this.updateSelectValue(this.CrmUsersIds)
        this.updateCurrentSelected(this.CrmUsersData)
      },
      // 选择行业
      showPositionData() {
        // 重置数据
        ...
        // 页面存在多种层级类型的选择时，才需要修改已选择的数据
        this.updateSelectValue(this.PositionIds)
        this.updateCurrentSelected(this.PositionData)
      }
    }
  }
</script>
