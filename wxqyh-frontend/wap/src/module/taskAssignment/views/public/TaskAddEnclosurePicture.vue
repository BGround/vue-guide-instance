<template>
  <div class="qwui-enclosure_pic_box">
    <div class="qwui-enclosure_pic_item">
      ...
      <div class="qwui-flexItem qwui-enclosure_pic">附件上传</div>
      ...
      <label v-show="false" ref="input_two" class="qwui-enclosure_pic_right">
        <input ref="input" type="file" accept="image/jpeg,image/png,application/vnd.ms-works,application/vnd.ms-excel,application/msword,aplication/zip,application/vnd.ms-powerpoint,application/pdf" class="qwui-w_zero" @change="mediaFileUpdate">
      </label>
      <label class="qwui-enclosure_pic_right" ref="enclosure_pic" v-if="!isMobile && hasPermission">
        <input ref="input" v-show="false" type="file" accept="image/jpeg,image/png,application/vnd.ms-works,application/vnd.ms-excel,application/msword,aplication/zip,application/vnd.ms-powerpoint,application/pdf" class="qwui-w_zero" @change="addEnclosurePic2">
      </label>
    </div>
    <div>
      <ul>
        <!--呈现图片数据-->
        <li class="qwui-enclosure_pic_li" v-for="(item,index) in newList" :key="index">
          <div v-if="item.src" class="qwui-enclosure_pic_show qwui-flex">
            ...
            <div class="qwui-enclosure_pic_iblock qwui-flexItem">
              ...
              <div class="qwui-enclosure_pic_size">{{item.fileSize}}</div>
						...
						<!-- v-if="hasPermission" -->
            <i class="qwui-enclosure_pic_item_right" v-if="showDealer && status=='0'" :class="enclosurePicDelClass" @click="doDelLi(item,index,$event.target,'delImg');"></i>
          </div>
