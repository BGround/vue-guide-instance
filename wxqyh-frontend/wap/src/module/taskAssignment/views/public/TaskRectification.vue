<template>
	<div class="qwui-enclosure_pic_box">
		<div class="qwui-enclosure_pic_item">
			...
			<div class="qwui-flexItem qwui-enclosure_pic">整改情况描述</div>
		</div>
		<div style="padding: 0 18px;">
			<div class="textAare-box" v-if="!showTextArea" @click="toggle">
				<span :style="{color: textVal!=''?'':'#bbb'}">{{textVal!=''?textVal:'请输入整改情况描述'}}</span>
			</div>
			<textarea class="textAare-box" v-else ref="rectification" v-model="textVal" :disabled="disabled" placeholder="请输入整改情况描述" maxlength="2048"
