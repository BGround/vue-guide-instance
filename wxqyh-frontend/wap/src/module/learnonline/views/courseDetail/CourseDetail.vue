<template lang="html">
  <div class="wrap qwui-courseDetail" :style="{ height: winHeight + 'px' }">
    <div class="scroll_wrap" :style="{ height: scrollHeight }" @scroll="scrollList" ref="scroll_wrap">
      ...
      <div v-else key="directory" class="directory_wrap">
        <div
          ...
          <!-- 章节头 -->
          <div class="chapter_bar flex_box" @click="slideUpDown(chap_index)">
            <p class="chapter_name flex_item ellipsis">
              {{chapter.chapterName}}
            </p>
            <div class="chapter_state">
    ...
    <!--图片预览组件-->
    <preview-images :useWxPreview="false" @hidePreviewImg="hidePreviewImg"></preview-images>

    <!-- 防作弊弹框组件 -->
    <cheat-tip :showTip="preventCheat.pauseTime" :showTipAndroid="preventCheat.android" :cheatTime="preventCheat.courseCheatTime" @restoreTimer="restoreTimer"></cheat-tip>
...
export default {
  ...
  data() {
    return {
      ...
      preventCheat: {
        timer: null,//防作弊定时器
        courseCheatTime: 0,//后台设置的课程防作弊时长，大于0的说明开启防作时长
        time: 0,//防作弊定时器已记录的时长
        pauseTime: false,//暂停图片学习的计时
        android: true,//为了安卓全屏状态下始终不现实弹框
      },
      previousMyState: 0, // 记录之前的进度（用于预览页面回退  0初始化未赋值 1学习中 3未学习 5已学习  => myState
    };
  },
  computed: {
    // 课程信息
  ...
  created() {
    ...
    getCoursePromise = new Promise((resolve, reject) => {
      getCourseDetailData({courseId: this.$route.query.id}, data => {
        ...
        // 如果是从预览课件跳转过来，核查上一次的进度，未完成的则需要判断当前是否完成
        // 如果有之前的学习进度，核对学习进度，从未完成->完成时，有考试。直接跳转考试
        // 即：预览前未完成-预览后来到本页面完成
        if ((this.previousMyState == 1 || this.previousMyState == 3) && data.personCourse.myState == 5) { // 1学习中 3未学习 5已学习
          this.autoEnterExam(true);
        }

        //整合防作作弊时长
        if(data.personCourse.cheatTime>0){
          this.preventCheat.courseCheatTime = data.personCourse.cheatTime * 60;
        }
  ...
  methods: {
    ...
    //预览图片课件
    preview() {
      EventBus.$emit('previewImages',(_.compressURL+this.imgUrl).replace('/compress/','/'),this.imgUrls);
    },
    // 显示隐藏章节
    slideUpDown(index) {
    ...
    // 进入考试
    enterExam(examId) {
      this.$router.push({name: 'exam_detail', params: {id: examId}});
    },
...
/*=======================以上方法基本不用改=====================================*/
    ...
    filePreview(par_index, index, event) {
      ...
      this._getFileType(courseware.enclosureId).then(() => {
        ...
        // 防作弊定时器
        if(this.preventCheat.courseCheatTime>0){
          clearInterval(this.preventCheat.timer);
          this.preventCheat.timer = setInterval(() => {
            //视音频在播放或者学习图片
            if((this.player && this.player.playing()) || courseware.fileType === 1){
              this.preventCheat.time++;
            }
            if(this.preventCheat.time > this.preventCheat.courseCheatTime && !this.preventCheat.pauseTime){
              //视音频退出全屏
              if(courseware.fileType === 2 || courseware.fileType === 3){
                //pc端退出全屏
        ...
        if (courseware.fileType === 2 || courseware.fileType === 3) {
          ...
          getUserFlow({}, data => {
            ...
            if (courseFlow <= totalFlow) {
              ...
              getVideoUrl({
                ...
                let canCheckFlag = false; // 可以检查进度的标志
                // 清空原音视频并清除原定时器
                ...
                this.player = new TcPlayer('preview_video', {
                  ...
                  "x5_player": true,//是否启用 TBS 的播放 flv 或 hls 。启用时播放器将在 TBS 模式下(例如 Android 的微信、QQ浏览器）将 flv 或 hls 播放地址直接赋给 <video> 播放
                  "x5_type": 'h5',//开启同层播放，防止安卓系统下的视频层级最高问题
                  "systemFullscreen": true,//开启后，在不支持 Fullscreen API 的浏览器环境下，尝试使用浏览器提供的 webkitEnterFullScreen 方法进行全屏
                ...
                this.$nextTick(() => {
                  ...
                  video.addEventListener("x5videoenterfullscreen", function(){
                    self.preventCheat.android = false;

                  })
                  video.addEventListener("x5videoexitfullscreen", function(){
                    self.preventCheat.android = true;
                  })
                });
                ...
                this.countTimer = setInterval(() => {
                  ...
                  if (this.player.playing()) {
                    canCheckFlag = true; // 开始后便赋值，在暂停时可以核查一次进度
                    this.timeNums++;
                  } else if(canCheckFlag && !this.player.fullscreen() &&
                            this.player.duration() - this.player.currentTime() == 0) { // 没在播放则判断课程是否完成
                    canCheckFlag = false; // 锁住，防止暂停后多次判断
                    // 视频跳转的条件：1 非全屏； 2 播放完，剩余0进度； 3 暂停； 4 之前未完成，现在完成；
                    this.autoEnterExam(); // 第4点在方法内统一做了
                  }
          ...
          if(courseware.fileType === 6 && isHttps && !(_.isWeChat() || _.isQiyeweixin()|| _.isHefeixin())){
            ...
            // 存学习进度 用于返回后跳转 // 如果有考试再赋值
            if (this.examData && this.examData.list[0] &&
                this.examData.list[0].examId && this.subject.myState != 5) { // 已完成的就不用存了
              sessionStorage.setItem('learnonline_course_'+ this.$route.query.id, this.subject.myState);
            }
            //跳转到预览组件
            this.$router.push({
              name: 'learn_detail',
              params: {
                ...
                enclosureId: courseware.enclosureId,
                cheatTime : this.preventCheat.courseCheatTime
              }
            });
          }
        }
    ...
    pauseTimer () {
      ...
      this.player && this.player.playing() && this.player.pause();
    },
    // 防作弊恢复学时时长记录
    restoreTimer () {
      //点击在之后防作弊时长重置
      ...
      this.player && !this.player.playing() && this.player.play();
    },
    // 关闭预览图片
    hidePreviewImg() {
      this.autoEnterExam();
    ...
    // 判断初次学习进度100%可自动跳转考试
    autoEnterExam(ignore = false) { // 可忽略当前已完成条件直接跳转
      // 两种判断，ignore：flase， 进来未完成，当前已完成
      //                ：true，  忽略条件（建立在预览课件pdf。。。 跳转缓存的基础上判断ok了才会传true）
      let examId = this.examData && this.examData.list[0] && this.examData.list[0].examId;
      // 如果进来未完成，现在已完成则可以跳转
      let flag = ignore? true: this.subject.myState != 5 && this.checkProgressCompleted();
      if (flag && examId) {
        // 设置sessionStorage 用于考试详情页内弹窗提示
        sessionStorage.setItem('learnonline_course_exam_'+ examId, 1);
        this.enterExam(examId); // 如果有考试就自动跳转考试
      }
    },
    // 检验是否全部进度完成
    checkProgressCompleted() {
      let chapterList = this.chapterList;
      let chapterListLength = chapterList.length;
      let getProgress = this.$options.__proto__.filters.getProgress;
      for (let i = 0; i < chapterListLength; i++) { // 章节
        let enclosureList = chapterList[i].enclosureViewVOList;
        let enclosureListLength = enclosureList.length;
        for (let j = 0; j < enclosureListLength; j++) { // 课件
          let nowEnclosure = enclosureList[j];
          if (getProgress(nowEnclosure) < 100) {
            return false;
          }
        }
      }
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      // 如果是从learn_detail 课件预览跳转过来的，判断之前的学习进度是否完成，并赋值给data
      let key = 'learnonline_course_'+ vm.$route.query.id;
      let storage = sessionStorage.getItem(key)
      if (from.name == 'learn_detail' && storage) {
        vm.previousMyState = storage; // 获取缓存进度 myState 状态  1学习中 3未学习 5已学习
