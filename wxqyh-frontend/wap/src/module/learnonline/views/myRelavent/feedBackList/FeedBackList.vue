<template>
  <div class="feedback-wrap">
        ...
        <a :href="detailUrl + item.id">
          ...
          <div class="fb-item-info">
            ...
            <p class="fb-item-other">
              <span class="fb-item-time">{{feedbackTime(item.createaTime)}}</span>
              <span>{{item.commentNum || 0}}回复</span>
            </p>
          </div>
        </a>
      </li>
    ...
    ></load-more>
    <div class="fb-button">
      <a :href="href" class="fb-btn-inner">我要反馈</a>
    </div>
  </div>
</template>
...
import { feedbackList, managesetting } from "../../../api/getData";
export default {
  data() {
    return {
      isLoading: true,
  ...
  methods: {
    initList() {
      let chargeName = this.$route.query.charge || "胡韵静";
      let appName = this.$route.query.app || "培训学习";
      var _this = this;
      _.showLoading();
      feedbackList({}, data => {
        _.hideLoading();
        this.isLoading = false;
        this.feedbackUrl += data.feedBackVersionId;
        if (_.isAdministrator) {
          this.href = `${this.feedbackUrl}&field=${
            wxqyhConfig.orgConfigInfo.corpId
          }&field1=${_.userId}&field2=${chargeName}&field3=${
            _.isAdministrator == 1 ? "管理员" : "普通员工"
          }&field4=${_.is_vip ? "付费用户" : "普通用户"}&field5=${appName}`;
        } else {
          managesetting({}, data => {
            _.isAdministrator = data.isManager;
            this.href = `${this.feedbackUrl}&field=${
              wxqyhConfig.orgConfigInfo.corpId
            }&field1=${_.userId}&field2=${chargeName}&field3=${
              _.isAdministrator == 1 ? "管理员" : "普通员工"
        ...
        this.list = this.filterList = data.tbFormWorkOrderPOList;
      });
    },
    searchFocus() {
      this.isFocus = true;
    ...
    searchFeedback() {
      ...
      this.filterList = this.list.filter(
        item => item.instanceTitle.indexOf(this.searchMsg) != -1
      );
    },
    clearInput() {
      this.searchMsg = "";
  ...
  .fb-list {
    ...
    .fb-list-item {
      ...
      .fb-list-icon {
        ...
        width: 24px;
        height: 24px;
        background: url(~assets/images/reim/icon_feedback@2x.png) center
          no-repeat;
