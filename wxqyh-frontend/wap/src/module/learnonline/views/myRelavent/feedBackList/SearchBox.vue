  ...
  <div class="qwui-search_wrap"  :class="{'qwui-search_mask': setTopMask, 'qwui-no_mask':searchinput.ridMask}">
    <div class="qwui-search_bar">
        ...
        <div class="qwui-search_inner" :class="{'qwui-with_left_span':searchinput.moreFunction && searchinput.moreFunction.show}">
          ...
          <!--输入按钮让移动端识别为搜索-->
          <form action="javascript:;" class="search_input_form">  
            <input class="qwui-search_input " 
              v-model="getKeyWord" 
              type="search" 
              @click="clickInput($event.target)"
              v-focus="inputFocus" 
      ...
      <!--不聚焦遮罩层-->
        <label class="qwui-search_text" :class="{'qwui-with_left_span':searchinput.moreFunction && searchinput.moreFunction.show,
         'with_right_span': searchinput.rightSpan && searchinput.rightSpan.show}" @click="closeThis()" v-if="showMask">
          ...
          <span style="font-size: 14px;margin-left: 4px; vertical-align: middle; color: #CCCCCC;">{{placeholder}}</span>
        </label>
    ...
    <!--聚焦白色图层-->
    <div @click="closeDialog" class="qwui-full_width">
      <p class="search-tips" v-show="setTopMask">{{tips}}</p>
    </div>
  </div>
</template>
    ...
    created:function(){
      ...
      if (this.searchinput && moreFunction){
        ...
        this.initChoose = moreFunction.option && moreFunction.option.length? moreFunction.option[initIndex].name:
                          moreFunction.defaultChoose? moreFunction.defaultChoose: this.$('i18n.pleaseSelect');
...
<style type="text/css" scoped>
  ...
  .qwui-search_inner .icon_search {
    ...
    background: url(~assets/images/reim/ic_search.png) right no-repeat;
    background-size: 14px;
    cursor: pointer;
  ...
  .qwui-search_hanber{
    ...
    margin: 6px 0 0 17px;
    padding: 0;
    /* background: url(~assets/images/icon_a-shearchbar_filter@2x.png) 0 -24px no-repeat; */
    background-size: 48px 36px;
    background-position: center;
    cursor: pointer;
  }
  .qwui-search_bar:after {
    display: none;
  }
</style>
