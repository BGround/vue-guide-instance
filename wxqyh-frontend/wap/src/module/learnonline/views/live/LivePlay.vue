...
export default {
  ...
  components: {
	...
	data() {
      ...
      getCommentTimer: null, //获取评论以及直播人数
    ...
    this.liveId = this.$route.query.id;
		this.getLivePlay(this.liveId).then(() => {
		  ...
		  if(corp_id == 'wx07ad350b0dfd7747') { //在欢迎你或者register开启
        var player = new TcPlayer('id_video_container', {
          "m3u8": `https://${this.livePlayData.liveplay}/live/${this.livePlayData.liveId}.m3u8`, //请替换成实际可用的播放地址
          "width" :  "100%",//视频的显示宽度，请尽量使用视频分辨率宽度
          "height" : "210",//视频的显示高度，请尽量使用视频分辨率高度
          "live": true, //直播
          "flash": false, // 使用h5播放，不使用flash
          "x5_player": true, //启用时播放器将在 TBS 模式下(例如 Android 的微信、QQ浏览器）将 flv 或 hls 播放地址直接赋给 <video> 播放
          "x5_type": "h5", // 通过 video 属性 “x5-video-player-type” 声明启用同层 H5 播放器
        ...
        const player = new qcVideo.Player("id_video_container", {
          "live_url": `http://${this.livePlayData.liveplay}/live/${this.livePlayData.liveId}.m3u8`,
          "width": 480,
          "height": 320,
          // "x5_type": "h5",
          "x5_fullscreen": false
      ...
      this.loadComment(1);
      this.getCommentTimer = setInterval(() => {
        this.loadComment();
        this.getLivePlay(this.liveId).then(() => {
          //更新在线人数
          this.onlineNum = this.livePlayData.online;
  ...
  // 离开页面前清除定时器
    ...
    clearInterval(this.getCommentTimer);
		next();
	},
	methods: {
		...mapActions([
    ...
    // 预览图片
    comPreviewImg(event) {
      EventBus.$emit('previewImages', event.target.currentSrc,this.previewURL);
    }
	},
}
</script>
