<template>
  <div class="qwui-lean_search_practice">
    <!-- tab选项卡 -->
    <header class="qwui-menu_tab_fixed fixed_set_top">
      <menu-tab-two
...
export default {
  ...
  methods:{
    ...
    // 进入练习详情页
    enterDetal(examId) {
      this.$router.push({name: 'practice_detail', params: {id: examId}});
    },

    // 进入练习页
    enterExam (exam) {
      if (this.getButtonText(exam) == this.$t('i18n.practiceNotStarted')) return;
      window.location.href=window.parseURL(_.baseURL+"/jsp/wap/examination/practice/practice_answer.jsp?id=" + exam.id);
    },
