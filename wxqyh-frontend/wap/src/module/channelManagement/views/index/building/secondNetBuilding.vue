        ...
        <div :key="'list'+index"
          ...
          <div class="list__info">
            <div class="list_header">
              <div class="list__name text-ellipsis">{{ item.secondNetName||'-'}}</div>
              <span class="list_id text-blue">{{item.manageCode||'-'}}</span>
            </div>
            <div class="list__data justify-between text-blue">
              <span class="icon_address text-ellipsis">{{item.area||'-'}}</span>
              <span class="icon_tag text-ellipsis">{{item.category||'-'}}</span>
              <span class="icon_tag text-ellipsis">{{item.buildNature||'-'}}</span>
...
<script>
  import {getSecondBuildManageByDealerId} from '@/module/channelManagement/api/building'
  ...
  export default {
    ...
    methods: {
      ...
      fetchSecondBuildManageByDealerId() {
        return getSecondBuildManageByDealerId({
          ...
          if (data && data.length) {
            data.forEach((item, index) => {
              ...
              if (item.showroomFrontImage) { // 展厅正面照片
                item.imageList.push(item.showroomFrontImage)
              }
              if (item.showroomFiveImage) { // 展厅45°照片
                item.imageList.push(item.showroomFiveImage)
              }
              if (item.showroomInsideImage) { // 展厅内部照片
                item.imageList.push(item.showroomInsideImage)
              }
              if (item.salesReceptionImage) { // 销售接待台照片
                item.imageList.push(item.salesReceptionImage)
              }
              if (item.serviceStationImage) { // 服务站正面照片
                item.imageList.push(item.serviceStationImage)
              }
              if (item.receptRoomImage) { // 接待室照片
                item.imageList.push(item.receptRoomImage)
              }
              if (item.loungeImage) { // 休息室照片
                item.imageList.push(item.loungeImage)
              }
              if (item.workshopImage) { // 车间照片
                item.imageList.push(item.workshopImage)
              }
            })
            this.list = data || []
          } else {
      ...
      initData() {
        ...
        const p1 = this.fetchSecondBuildManageByDealerId()
        this.isLoading = true;
