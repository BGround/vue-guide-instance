<template>
  <div class="detailBox">
    ...
    <div>
      <div v-if="companyVO">
        <div class="topContent topContentPc">
          ...
          <p class="tip">
            ...
            <a class="seeAll-history" href="#viphistory" @click="vipBuyHistory.show = true">查看全部</a>
          </p>
      ...
      <div class="msgCard msgCardPc" id="viphistory">
        <p class="pcTitle">
          <span>vip信息<span>（历史购买）</span></span>
          <a class="clickIcon" @click="vipBuyHistory.show=!vipBuyHistory.show"></a>
        </p>
        <div class="tableScroll" @mousedown="moveThis($event)" v-show="vipBuyHistory.show">
          <div >
            <ul class="tableTitle">
              <li class="bgGrey"
                v-for="(item,index) in vipBuyHistory.headerTitle"
                :key="index">
                {{item}}
              </li>
            </ul>
          ...
          <div>
            <ul class="tableContent fontSize-none" v-for="(item,index) in vipBuyHistory.list"
                :key="index" :class="{bgGrey: index%2 == 1}"> <!--  备注信息  -->
              <li :title="item.vipName">{{item.vipName}}</li>
              <li>{{item.getSource}}</li>
              <li>{{item.personMax}}</li>
              <li>{{item.vipStartTime}}</li>
              <li>{{item.vipEndTime}}</li>
            </ul>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    data(){
      return {
        ...
        vipBuyHistory:{
          show: false,
          headerTitle:['VIP类型', '接入方式', '人数', '开始时间','结束时间'],
          list: []
        }
      }
    },
    ...
    created() {
      getCompanyDetail(this.corpId, (data)=> {
        ...
        this.historyVipInfo = data.historyVipInfo
          ...
          //VIP购买记录
        if(data.companyVO && data.companyVO.vipDataVOList && data.companyVO.vipDataVOList.length){
           this.vipBuyHistory.list.push(...data.companyVO.vipDataVOList)
        }
    ...
    methods: {
      ...
      addInfoHandle(item,index) {
        ...
        if(!item){
          ...
          this.experienceInfoPO["corpId"]=this.$route.params.corpId;
        }
      },
      cancelDialog() {
      ...
      confirmDialog() {
        ...
        operationRemarks(this.experienceInfoPO, function (data) {
          ...
          self.experienceInfoPO = {
            corpId: self.$route.params.corpId,
            id: '',
          }
        },function () {
          self.experienceInfoPO = {
