<template>
  <div class="feedback-wrap">
        ...
        <a :href="detailUrl + item.id">
          ...
          <div class="fb-item-info">
            ...
            <p class="fb-item-other">
              <span class="fb-item-time">{{feedbackTime(item.createaTime)}}</span><span>{{`${item.commentNum || 0} ${$t('i18n.comment')}`}}</span>
            </p>
          </div>
        </a>
      </li>
    ...
    <div class="fb-button">
      <a :href="href" class="fb-btn-inner">{{$t('i18n.toFeedback')}}</a>
    </div>
  </div>
</template>
...
import {feedbackList, isAdministrator} from "../api/more.js"
export default {
  data () {
    return {
      ...
      isFocus: false,
      once: true,
      feedbackUrl: `${location.href.slice(0, location.href.indexOf('/vp'))}/vp/module/form.html?agentCode=reimbursement&corp_id=${wxqyhConfig.orgConfigInfo.corpId}#/open/add?id=`, //正式
      detailUrl: `${location.href.slice(0, location.href.indexOf('/vp'))}/vp/module/form.html?agentCode=reimbursement&corp_id=${wxqyhConfig.orgConfigInfo.corpId}#/open/detail?id=`,   //正式
      href: '',
      list: [],
      filterList: [],
    };
  ...
  computed: {
    ...
    loadText() {
      return this.$t('i18n.total') + this.filterList.length + this.$t('i18n.dataTotal');
    }
  },
  ...
  methods: {
    initList() {
      ...
      feedbackList().then(res => {
        ...
        if (res.data.feedBackVersionId) {
          this.feedbackUrl += res.data.feedBackVersionId;
        }

        if(_.isAdministrator) {
          this.href = `${this.feedbackUrl}&field=${wxqyhConfig.orgConfigInfo.corpId}&field1=${_.userId}&field2=邝芷欣&field3=${_.isAdministrator == 1 ? '管理员' : '普通员工'}&field4=${_.is_vip ? '付费用户' : '普通用户'}`;
        }else {
    ...
    feedbackTime(stamp) {
      ...
      //     date = time.getDate() < 9 ? ('0' + time.getDate()) : time.getDate();
      // if(currentYear === time.getFullYear()) return `${month}-${date}`;
  ...
  .fd-search-box {
    ...
    .fd-search-form {
      ...
      .fd-search-icon {
        ...
        height: 26px;
        margin-left: -30px;
        background: url(~assets/images/reim/ic_search.png) no-repeat;
      ...
      .fd-delete-icon {
        ...
        width: 26px;
        height: 26px;
        background: url(~assets/images/reim/ic_del.png) no-repeat;
  ...
  .fb-list {
    ...
    .fb-list-item {
      ...
      .fb-list-icon {
        ...
        width: 24px;
        height: 24px;
        background: url(~assets/images/reim/icon_feedback@2x.png) center no-repeat;
