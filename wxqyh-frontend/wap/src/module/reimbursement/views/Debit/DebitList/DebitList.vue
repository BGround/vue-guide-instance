<template lang="html">
  <div class="reimbursement">
    ...
    <qw-advance-search-box
      ...
      @searchChange="showAdvancedSearch"
    ></qw-advance-search-box>
...
<script>
  ...
  export default {
    ...
    created() {
      if(this.getStore('debitList')){
        this.statusText = this.$route.meta.statusText || this.getStore('debitList').statusText;
        this.queryType = this.$route.meta.queryType=='0'? this.$route.meta.queryType : this.getStore('debitList').queryType;
        this.cacheSearchValue = this.getStore('debitList').cacheSearchValue;
      }else {
      ...
      if(this.$route.meta.statusText){
        delete this.$route.meta.statusText;
        delete this.$route.meta.queryType;
      }
    },
    beforeDestroy () { // 组件销毁前缓存高级搜索条件
      ...
      this.setStore('debitList', searchObj);
    },
    methods: {
      // 页面滚动时头部导航的显示和隐藏
      scrollHandle(e){
