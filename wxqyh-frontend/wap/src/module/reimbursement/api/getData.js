...
export function getOldccOrTolist(type, callback) {
  _.showLoading();
  axios.post(_.baseURL + '/portal/reimPortalCtl/getOldGivenList.do', {
    type: type
  }).then(function (result) {
    if (result.code === '0') {
      _.hideLoading('');
...
// 加载借款上次审批人、相关人
export function getOldccOrTolistLoan(type, callback) {
  _.showLoading();
  axios.post(_.baseURL + '/portal/loan/loanPortalCtl/getOldGivenList.do', {
    recipientType: type
  }).then(function (result) {
    if (result.code === '0') {
      _.hideLoading('');
