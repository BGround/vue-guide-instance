  ...
  <div class="wrap">
    ...
    <div class="searchResult"
      ...
      <div v-for="(item,index) in examData" :key="index" class="items">
          ...
          <div class="content_padding">
            <div class="listContent">
              <div class="listContentMsg">
                <p>联系信息:{{item.callPersonName | noneMsg}} <span class="separate">|</span> {{item.lastCallTime | timeRecord}}{{item.callCount | callCount}}</p>
              </div>
              <transition name="fade">
                <div class="listContentMsg">
                  <p>客户信息:{{item.contactName | noneMsg}} <span class="separate">|</span> {{item.regisTime | noneMsg}}(接入)</p>
                  <p>企业信息:{{item.staffNumber | noneMsg}} <span class="separate">|</span> {{item.industry | noneMsg}} - {{item.corpSubIndustry | noneMsg}}</p>
                </div>
              </transition>
              <transition name="fade">
                <div class="listContentMsg">
        ...
        <div v-if="tabShow">
          <div class="qwui-btns">
            <div class="item" v-for="(btnItem,btnIndex) in btns" :class="{borderRight:btnIndex!=btns.length-1}" :key="btnIndex" @click="showDialog(btnIndex,item,index)">
              {{btnItem.text}}
            </div>
          </div>
        </div>
    ...
    <user-select :selectconfig="dataBase.selectConfig"
                 @chooseDone="chooseDone(chooseType)"
                 closeAllTab
                 closeGroup
                 closeDept
                 tabActive="user"
...
<script>
  ...
  import {getCall,getRemarkInfo,remarkInfo,classify,returnCustomer,transferCustomers} from '../api/getData'
  ...
  export default {
    ...
    data(){
      return {
        ...
        footer:[
          {text:"公海池(0)", index:0, num:0},
          {text:"客户池(0)", index:1, num:0},
          {text:"我的(0)", index:2, num:0}
        ],
        loadMore:{//加载更多组件
        ...
        nowItem:{}, //存储点击的item
        indexControl:0,
        examIndex:0,//点击的列表项索引
        chooseType:0,//选人回调类型  0：选人，1:转移客户
        firstEntry:true,//第一次进入页面
        recordHeight:0,
        searchShow:false//搜索栏是否显示
      }
    },
    computed: {
    },
    ...
    methods: {
      ...
      backToCall:function () {
        ...
        this.examData = [];
        if((this.footer.length==3&&this.footerIndex==1)||(this.footer.length==2&&this.footerIndex==0)){
          this.loadMore.data.flag=3;
        }
      ...
      showMsgFromLoadMore:function (val) {
        if(val.totalRows && !this.totalRows.isFirstTime){
          ...
          this.footer[0].num = val.totalRows;
        ...
        val.myCallExperience ? (this.footer[this.footer.length - 1].text = '我的('+val.myCallExperience+')'):'';
        val.myCallExperience ? this.footer[this.footer.length - 1].num = val.myCallExperience:0;
        if(val.ClientCount){
          this.footer[this.footer.length - 2].text = '客户池('+val.ClientCount+')';
          this.footer[this.footer.length - 2].num = val.ClientCount
        }
        if(val.pageData) {
        ...
        //判断销售人员是否拥有权限，是否显示tabs栏
        if(this.firstEntry){
          this.cannotCall = true;
          if(val.hasPower==-1){
            this.footer.length == 3 ? this.footer.splice(0,1) : ''
            this.loadMore.data.flag = 3;
          }
          _.hideLoading();
      ...
      //选人完成，触发搜索
      chooseDone:function (chooseType) {
        if(chooseType==0){
          ...
          transferCustomers(this.nowItem.corpId,this.dataBase.selectConfig.selectList["ccPersonList"].userSelected[0].userId,()=>{
            this.examData.splice(this.examIndex,1);
            this.tabs[this.indexControl].num--;
            this.footer[this.footer.length - 1].num--;
            this.footer[this.footer.length - 1].text = '我的('+this.footer[this.footer.length - 1].num+')';
            if(this.loadMore.padeConfig.totalRows=="1"){
              this.loadMore.padeConfig.totalRows="0";
            }else{
              this.loadMore.padeConfig.totalRows--;
            }
          })
        }
      },
      ...
      showMsgFromSearchBox(keyWord, val, isOption){
        ...
        if(this.footerIndex != this.footer.length - 1 || (val && val == 2)){
          this.tabShow = false;
          this.loadMore.data.userId = '';
          this.loadMore.data.level = '';
        }else{
      ...
      confirmDialog(){
        ...
        if(this.dialogContent.remarkShow){
          ...
          let levelIndex = (this.radio)*1;
          if(this.indexControl==levelIndex){
            return;
          }else{
            classify(val.callInfoId,levelIndex,()=>{
              this.nowItem = {};
              this.radio = "1";
              this.examData.splice(this.examIndex,1);
              this.tabs[levelIndex].num++;
              this.tabs[this.indexControl].num--;
              if(this.loadMore.padeConfig.totalRows=="1"){
                this.loadMore.padeConfig.totalRows="0";
              }else{
          ...
          returnCustomer(val.corpId,()=>{
            this.nowItem = {};
            this.examData.splice(this.examIndex,1);
            this.footer[this.footer.length - 2].num++;
            this.footer[this.footer.length - 2].text = "客户池("+this.footer[this.footer.length - 2].num+")";
            this.footer[this.footer.length - 1].num--;
            this.footer[this.footer.length - 1].text = '我的('+this.footer[this.footer.length - 1].num+')';
            this.tabs[this.indexControl].num--;
            if(this.loadMore.padeConfig.totalRows=="1"){
      ...
      //弹出框弹出事件
      showDialog(index,val,listIndex){
        this.nowItem = val;
        this.examIndex = listIndex;
        switch(index){
          case 0:
