    ...
    data(){
      return {
        ...
        hasRecord:{
          ...
          text:"i18n.noRepTextApprove"
        },
        selectData:{
          index:0,
          nav:[ //我的申诉 tab
            {
              content: 'i18n.waitingApprove',
              num:'',
              data:{ //点击切换传递的数据
                page:1,
                pageSize:20,
    ...
    methods:{
      ...
      showMsgFromLoadMore(data){
        if(this.selectData.index == "0"){
          this.selectData.nav[this.selectData.index].num = data.totalRows?('(' + data.totalRows +')'):'';
