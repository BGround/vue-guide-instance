...
<div class="wrap" v-show="showContent">
  ...
  <div class="qwui-express_detail_content">
    ...
    <div class="qwui-express_detail_annotation" v-if="requestData.receiveRefUser">
      ...
      <span class="qwui-express_detail_icon qwui-express_icon_other" v-if="requestData.status=='1' && requestData.lastType=='2'"></span>
    </div>
    ...
    <div class="qwui-express_detail_markTime" v-if="requestData.status=='1'">
      <div class="qwui-express_detail_timeOut">{{(requestData.status=='1' && requestData.lastType!='0')? "领取时间":"关闭时间"}} :<span>{{requestData.receiveTime.substring(0,16)}}</span></div>
    </div>
  </div>
  <!--操作按钮-->
    ...
    methods:{
            ...
            //处理按钮显示
              ...
              if(self.requestData.status=="0"){
                if(result.data.duser=='0'){
                  ...
                  self.dealButton("primary","确认已领取",self.updateExpressStatus);
                } else if(result.data.duser=='2' && self.requestData.status == '0'){
                  self.isRefUser = true;
                  self.dealButton("primary","确认代领",self.updateExpressStatus);
                }
              }
              //返回按钮
      ...
      updateExpressStatus(){
        ...
        _.ajax({
          ...
          success: function (result) {
            _.hideLoading();
            _.tooltips_succeed(self.isRefUser?"代领成功":"领取成功");
            self.isSubmitLoading=false;
            if (result.code == '0') {
              self.requestData.buttonConfig={primaryList:[],defaultList:[]};
              self.requestData.status='1' ;
              self.requestData.lastType=self.isRefUser?'2':'1';
