<template>
  <div class="custom-form">
    <div v-show="!LoadCompletion" class="LoadCompletion"></div>
    <div class="topWarn" v-show="showTips">
        <div class="topWarn-text">{{topTipsText}}</div>
        <div v-show="noMobileApp" class="topIcons">
              <span class="printTop-item" v-show="openPrint" @click="openPrintHtml()">
                  <i class="icon-btn icon-btn-open"></i>
                  <a href="javascript:;">直接打开</a>
              </span>
          <span class="printTop-item tag-read" v-show="copyLink" @click="wxShare">
                  <i class="icon-btn icon-btn-copy"></i>
                  <a href="javascript:;">复制链接</a >
              </span>
          <span class="printTop-item" @click="printForm(this)">
                  <i class="icon-btn icon-btn-print"></i>
    ...
    <div class="print-footer" v-show="workOrderId && aginPrint"><span class="print-btn" @click="printForm">{{printBtnText}}</span></div>
  </div>
</template>
...
<script>
  ...
  import { designPrinttemp ,addPrintTemplet,getPrinttemp,setIsPrinted} from "../../api/printApi"
  export default {
    name: "custom_form",
    components: {
      HotTable,
    ...
    data() {
      var _this = this;
      var isEdit = this.$route.query.workOrderId ? false : true;
      return {
        size:'A4',//默认A4
        layout:'portrait',
        sizeMap:{
        ...
        printData: dataBase.printTemplate,
        cellList: [],//单元格数据
        dom: "",
        options: {
          rowHeight: 24,
        ...
        printBtnText: "打印",
        showTipsText: {}, //提示语方案
        showTips: false,//显示灰色div
        shareId: '',//分享Id
        shareUrl:'',//分享路径
        noMobileApp: true, //非移动端
        topTipsText:'分享给相关人员打印',
        openPrint: true,
        copyLink: true,
        aginPrint: false,//再次打印
        LoadCompletion: false,//预览样式
      };
    },
    created(){
      _.showLoading();
      ...
      if(this.workOrderId){
        //预览的时候
        getPrinttemp(this.workOrderId,this.$route.query.shareId).then(function(result){
          _.hideLoading();
          if(result.code == 0){
            ...
            _this.changeItemMap();
            _this.setValue(result.data.formPrintTemplet.templetHtml);
            ...
            _this.getShareUrl(result);
            _this.LoadCompletion = true;
            if(result.data.shareVO){_this.share(result.data.shareVO);}
          }else{
        ...
        designPrinttemp(this.definitionVersionsId).then(function(result){
          if(result.code == 0){
            _this.printTitle = result.data.controlPO.titleTemplate;
            _this.fields = result.data.fields;
            _this.sysFields = result.data.sysFields;
            _this.initFieldMap();
            _this.setValue(result.data.formPrintTemplet.templetHtml);
...
// 用户取消分享后执行的回调函数
      ...
      wxShare(){
        ...
        clipboard.on('success', e => {
          _.tooltips_succeed("已复制");
        })
        clipboard.on('error', e => {
          _.tooltips_succeed("复制失败");
        })
      },
      getShareUrl(res){
        this.shareId = res.data.shareId;
        var shareVO = res.data.shareVO
          ,canShare = res.data.canShare
        if(canShare){
          if(!_.isWeChatApp()){
            this.shareUrl = shareVO.shareUrl
          }
        }
        if (_.isWeChat()) {
          if (this.shareId) {
            this.showTips = true;
            if (_.isWeChatApp() || _.isQiyeweixinApp()) {//app
              this.topTipsText = '分享给相关人员打印';
              this.noMobileApp = false;
            } else {//微信pc/企业微信pc
              this.noMobileApp = true;
              this.openPrint = false;
              this.topTipsText = '可分享本页面进行打印，无权限的字段显示为空白单元格';
                if (_.isQiyeweixin()) {
                  this.copyLink = true;
                }else{
                  this.copyLink = false;
              ...
              if(_.isWeChatApp() || _.isQiyeweixinApp()){//手机端外部打开
                this.showTips = false;
              }else{//pc端外部打开
                this.showTips = true;
                this.noMobileApp = true;
          ...
          if (self.frameElement && self.frameElement.tagName == "IFRAME") {
            ...
            this.showTips = false;
            this.aginPrint = true;
            $('.print-table.preview').css("margin","0 auto 70px");
            if(_.isMobileApp()){
              this.aginPrint = false;
            }
          }
      ...
      printForm() {
        ...
        this.$nextTick(function(){
          ...
          if(_.isWeChat() || (self.frameElement && self.frameElement.tagName == "IFRAME")){
            this.showTips = true;
          }
        })
      },
...
//                e.clearCellData()
      ...
      saveTemplate(){
        var data = {definitionVersionsId: this.definitionVersionsId, templetHtml: JSON.stringify(this.getValue())}
...
//        });
      ...
      _setStyle(data) {
        if(data.type == 'page'){
          ...
          this.pageSize = this.sizeMap[this.size][this.layout];
          this.setPage();
        }
        if(data.type == 'layout'){
...
//              break;
            ...
            case "merge":
              ...
              break;
            default:
//              this._dealFontStyle(selectedCells, data.action)
          }
        }
      },
...
//              break;
       ...
       */
      drag(event, prop,parent) {
        this.clearCurrentCell();
        this.dom = {field:prop,parent:parent};
        event.target.style.borderColor = "rgba(248,123,0,0.5)";
        event.target.style.backgroundColor = "rgba(248,123,0,0.1)";
        event.dataTransfer.effectAllowed = "move";
      },
      dragenter(event) {
        if (event.screenX > 252 && event.screenY > 228) {
...
//          images: image,//image是背景图的数据
      ...
      setValue(e) {
        ...
        utils.isObjectEmpty(e) && (e = this._getDefaultValue()),
          ...
          this.dataMap = {},
          this.styleMap = {},
          this.size = e.page && e.page.size||'A4';
          this.layout = e.page && e.page.layout || 'portrait',
          this.pageSize = this.sizeMap[this.size][this.layout],
          this.fieldMap = {},
          this.borderState = {},
        ...
        t.CellStyleAction = function(e, t, n) {
          ...
          t.CellStyleAction.prototype.redo = function(t, n) {
            t.selectCell(this.range.from.row, this.range.from.col, this.range.to.row, this.range.to.col),
              e.setCellStyle(this.style, this.range),
...
<style>
  ...
  .icon-btn-open {
    background: url(~assets/images/ic_open.png) no-repeat scroll center center;
  }
  .icon-btn-copy {
    background: url(~assets/images/ic_copy.png) no-repeat scroll center center;
  }

  .icon-btn-print {
    background: url(~assets/images/ic_print.png) no-repeat scroll center center;
  }
  .topWarn {
    position: fixed;
    top: 0;
