...
const mutations = {
  ...
  addCCPersonList(state,payload) {
    ...
    payload.forEach(value=>{
      ...
      state.detailData.detail.ccPersons.push(value)
    })
  },
...
const actions = {
	...
	getMoveDetail({commit},{moveId,size=10,callback=function () {}}){
		axios.post(_.baseURL+'/portal/portalForm/getMoveDetail.do',{
			moveId,
			size,
		}).then((result)=>{
			_.hideLoading();
	...
	updataAuditStatus({commit,state},{...data}) {
		...
		_.showLoading('')
		axios.post(_.baseURL+'/portal/portalForm/updataAuditStatus.do',{//portal/portalForm/updataAuditStatus
			...commitData
		}).then((result)=>{
			_.hideLoading();
			if(result.code==="0"){
  ...
  //校验委托加签规则
  checkEntrustAddSignRule({commit,state},{...data}) {
	  let {callback,...commitData} = data;
	  _.showLoading();
	  axios.post(_.baseURL + '/portal/flow/flowEntrust/checkEntrustAddSignRule.do',{
      ...commitData
    }).then((result) => {
	    _.hideLoading();
	    if(result.code==="0") {
	...
	addCountersignType({commit,state},{isFree,type,addCounterSignPersonStr,reason,id,currentNodeId,orderVersion}) {
		_.showLoading('正在加签...')
		axios.post(_.baseURL+'/portal/portalForm/addCountersignType.do',{
			isFree,type,addCounterSignPersonStr,reason,id,currentNodeId,orderVersion,
		}).then((result)=>{
			_.hideLoading();
			if(result.code==="0"){
