...
const mutations = {
	...
	// 更新人员控件已选人
	updateItemKeysSelected(state,payload) {
    updateArray(state.FlowAuditUser[payload.index].startAuditList.userSelected, payload.data);
    updateArray(state.FlowAuditUser[payload.index].startAuditList.allUser, payload.data)		//已选中的所有人，不做增删处理
	},
	// 删除处理人
	...
	// 更新人员控件处理人
	updateFlowNodeItemKeys(state,payload) {
		state.flowNodeItemKeys = payload
	},
	// 保存节点的选人信息
	...
	saveChoiseFlowUserJson(state){
		...
		state.FlowAuditUser.forEach(function(value){
			...
			if(!users) return;
			// 允许上个节点指定处理人和//表单人员控件的代码
			if (value.isChoise == "0" || value.isChoise == "2"|| value.flowNodeItemKeys) {
				let ids = users.map((value)=>{
					return value.userId
				})
	...
	resetFlowData(state){
		...
		state.flowDpIdJson = ""
		state.flowNodeItemKeys = ""
		state.FlowAuditUser.length=0
		state.FlowAuditUser.push({
			startAuditList:DefaultFlowAuditUser
		})
		state.ccPersonList.length=0
		DefaultFlowAuditUser.userSelected.length = 0
	},
  //更新下拉选择的节点名称
  updateChooseNodeName(state){
    if(state.nextNodeList.length){
      ...
      state.choosedNodeName = chooseNodeTitle;
    }
  },
}
const actions = {
