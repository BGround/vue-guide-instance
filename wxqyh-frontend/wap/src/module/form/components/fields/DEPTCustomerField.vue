<template>
	<div class="field-content">
		...
		<div v-if="!field.disabled && !field.readonly" class="childForm-item-add" @click="addDept">点击添加</div>
	</div>
</template>
<script>
  import {field_validation} from './validation.js'
	export default {
		name: 'DEPTCustomerField',
		props: ['field'],
		created () {
		...
		methods: {
			...
			setDeptFormValue(value) {
				let deptlist = value||[];
				let depts = [];
				deptlist.forEach(item =>{
					let arr = item.split('_');
					let departmentName = arr.pop();
					let id = arr.join('_');
					depts.push({
						id:id,
						departmentName:departmentName
