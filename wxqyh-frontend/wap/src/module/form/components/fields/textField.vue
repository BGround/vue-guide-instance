	...
	<div class="field-content">
		<input v-if="!field.disabled" class="text-field" type="text" :value="field.value" :class="{'text-field-help':field.help,'readOnly':field.readonly || field.disabled}"
           ...
           :maxlength="maxLength"
		     ...
		     @blur="validate">
		<div v-else class="geo-field readOnly" :class="{'text-field-help':field.help}"><p class="text-field-p" v-html="filterTextContent(field.value)"></p></div>
		...
		methods:{
			setValue({target},field){
				...
				if(value.length == this.maxLength){
				  this.field.help = '最大长度为 '+ this.maxLength + ' 字';
        }
