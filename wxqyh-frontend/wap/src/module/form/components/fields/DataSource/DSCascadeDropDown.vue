  ...
  import {dataSource} from '../../../api/list'
    ...
    methods:{
      initData(){
        ...
        //下一个下拉框的数据
        var len = this.field.value?this.field.value.length:0;
        if(len && len != this.field.cascadeLevel &&this.fieldVal[len-1][this.id]){
          this.getData(this.fieldVal[len][this.id],(res)=>{
            _this.$set( _this.choiceData,len,_this.choiceData[len].concat(res))
          })
        }
      ...
      getData(keyword,callback){
        var _this = this;
        var url = this.field.ref_url,isParam = `wxUserId=${dataBase.config.wxUserId}&corpId=${wxqyhConfig.orgConfigInfo.corpId}&id=${this.field._id}&page=1&pageSize=50&keywordType=parentid&keyword=${keyword}`;
        url.indexOf('?') !=-1? url = url+ '&'+isParam: url = url+ '?'+isParam
      ...
      setValue(index){
        ...
        if(this.fieldVal[index-1][this.mainField]=='请选择'&&!this.fieldVal[index-1][this.id]){  //取消当前下拉框的选择时
          ...
          if(index == this.field.cascadeLevel){  //最后一个下拉框
            ...
            this.fieldVal.forEach(item =>{
              lastVal.push(JSON.stringify(item))
            })
            this.$emit('sourceData',lastVal)
          }else{
            this.getData(value[index-1][this.id],res=>{
              refChoiceData(index,res)
              var value = _this.fieldVal.slice(0,index),lastVal = [];
              value.forEach(item =>{
                lastVal.push(JSON.stringify(item))
              })
              _this.$emit('sourceData',lastVal)
            })
          };
        };
        ...
        function refChoiceData (index,res) {
          var fieldValTemp = _this.fieldVal.slice(0,index);
          var choiceDataTemp = _this.choiceData.slice(0,index);
          for(let i=index;i<_this.field.cascadeLevel;i++){
            fieldValTemp.push(_this.defaultData);
            choiceDataTemp.push([_this.defaultData]);
          }
          _this.fieldVal = fieldValTemp;
          choiceDataTemp[index].push(...res)
          _this.choiceData = choiceDataTemp;
        }
      }
    }
	}
