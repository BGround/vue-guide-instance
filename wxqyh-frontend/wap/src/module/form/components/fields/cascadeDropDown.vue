  ...
  <div class="field-content">
		<div class="drop-field">
			<select class="drop-down drop-down1"
				...
				<option v-for="choice in field.choices" :selected="choice._id == key || choice.value == first" :value="choice._id">{{(choice.value==='请选择'&&field.disabled)?'':choice.value}}</option>
			</select>
		</div>
    <div class="drop-field">
...
<script>
  ...
  export default {
    ...
    created(){
      this.field.choices[0]._id && this.field.choices.unshift({
        value:'请选择',
        _id:''
      })
      ...
      if(!field.value){
        ...
        this.first = arry[0].replace(this.key+'_','');
        this.val = arry[1];
      }
    },
		computed:{
			childChoices(){
				...
				if(this.key){
					for( var item of this.field.choices){
						if(item._id == this.key || item.value == this.first){
