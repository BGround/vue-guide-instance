<template>
  <div class="wrap" ref="wrap" v-windowscroll="scrollList" :class="{'qwui-flow_top':flowTip}">
    <!--无法流转提示-->
    ...
    <div  v-if="detail">
      <div class="form-bg">
        <div class="form_title_box">
          ...
          <div class="form-title-msg">
            ...
            <span class="form-title-span colorblue" @click.stop="showPersonDetail">{{personName}}</span>
      ...
      <formBase v-show="showFormBase" @historyTitle="paramHistoryData" @showSaveBtn="showSave = true" @setLinkage="toGetLinkage"  @setAdvmthd="toGetAdvmthd" ></formBase>
      <!--附件列表-->
      ...
      <!--上一页下一页-->
      <flow-button v-if="showOpenFlowBtn" :buttondata="buttonConfig"></flow-button>
      ...
      <div style="position: relative;" :class="{'circulationDetails':isShowSerial}">
        ...
        <user_selected_group
          ...
          <user_selected_item
            ...
            :isEdit="isEditShow"
            @atThisPersonUtil="atThisPersonUtil"
            @showUserSelectedBox="FlowAuditUserShowBox(0)"
            @showPersonList="showPersonList"
            @removeSelected="(payload)=>removeFlowAuditUser({delUserIndex:payload.index,index:0})"
            :isRemove="FlowAuditUser
                        && FlowAuditUser[0]
                        && FlowAuditUser[0].startAuditList
                        && FlowAuditUser[0].startAuditList.isCanEdit
                        && FlowAuditUser[0].startAuditList.isCanDelete">
          </user_selected_item>
          <div class="qwui-user_cell_title" slot="qwui-user_select_person_open"
               v-show="isShowFlowNodeItemKeys()">
            <p class="qwui-user_flowNodeItemKeys">
              已关联[{{isItemKeys(FlowAuditUser[0] && FlowAuditUser[0].startAuditList && FlowAuditUser[0].startAuditList.flowNodeItemKeys)}}]字段，请先填写
            </p>
          </div>
        </user_selected_group>
...
<script>
  ...
  import { compatible } from '../../js/compatible'  //导入fields的兼容性方法
  ...
  export default {
    ...
    data() {
      return {
        ...
        buttonConfig: { //操作按钮
          primaryList: [{type: "primary", name: '转内部流程处理', callBack: null}],
          defaultList: [],
          style: {class: ""}//按钮是否置底部 active :底部、"":相对定位
        },
        showSave:false, //保存按钮显示
        showOpenFlowBtn: false,// 转内部流程按钮显示
        imgUploadPicList:[],
    ...
    created() {
      ...
      this.buttonConfig.primaryList[0].callBack = function(){
        _this.$router.push({name: 'add', query: {id: _this.id,type:'edit'}})
      }
      var whiteCopidParam = {
        "agentCode": "form",
    ...
    computed: {
      ...
      nextAuditListShow() {
        if(this.flow.isover != '2' ) return false
        // 人员控件
        if(this.FlowAudit && this.FlowAudit.flowNodeItemKeys){
          return true;
        }
        let flag1 = this.FlowAudit && this.FlowAudit.nextAuditList.length>0 // 有下一步处理人
      ...
      isEditShow(){
        // 如果下一步处理人由人员控件决定时
        if(this.FlowAuditUser.length && this.FlowAuditUser[0].startAuditList && this.FlowAuditUser[0].startAuditList.flowNodeItemKeys){
      ...
      isListShow(){
        ...
        if(this.FlowAudit.nextAuditList){
          this.FlowAudit.nextAuditList.map((item) => {
        ...
        // 如果下一步处理人由人员控件决定时
        if(this.flow.isover == '2' && this.FlowAuditUser.length && this.FlowAuditUser[0].startAuditList && this.FlowAuditUser[0].startAuditList.flowNodeItemKeys){
          // 判断当前节点名称是否等于下一节点名称（判断是否有加签）
          if(this.FlowAudit && this.FlowAudit.vo && (this.FlowAudit.vo.nodeName == this.FlowAudit.nextNodeName)){
            return arr
          }
    ...
    methods: {
      ...
      flowNodeItemKeysShow() {
        // 如果下一步处理人由人员控件决定时
        if(this.FlowAuditUser.length && this.FlowAuditUser[0].startAuditList){
          if(this.FlowAuditUser[0].startAuditList.flowNodeItemKeys){
      ...
      // 人员控件提示显示
      isShowFlowNodeItemKeys() {
        return this.flow.isover == 2
               && this.FlowAuditUser[0]
               && this.FlowAuditUser[0].startAuditList
               && this.FlowAuditUser[0].startAuditList.flowNodeItemKeys
               && this.FlowAuditUser[0].startAuditList.allUser.length === 0;
      },
      //可编辑表单
      editTitle() {
      ...
      startAuditTitle(item) {
          return '当前节点 · ' + item.vo.nodeName;
      },
      // 下一节点处理人标题
      nextAuditTitle() {
        if(this.FlowAudit && this.FlowAudit.nextNodeName){
          return '下一节点 · ' + this.FlowAudit.nextNodeName;
        }else {
          return '下一步处理人'
        }
      },
      ...
      dealCommentList(data){
        this.loading=false;
        this.showCommentList(data.comments,data.hasMore,data.commentCount);
      },
      ...
      getFlowAuditDetail: function (result) { //当前节点的详细信息
        //如果已退审、已关闭或者审批完则nodeId传空
        if(this.flow.isover == 0 || this.flow.isClose == 1){
          this.flow.nodeId = '';
        }
        ...
        audit_flow_api.getFlowAuditDetail(dataParam, (result) => {
          ...
          this.carbonList = data.carbonList;
          if(data.carbonList.some(item => item.userId == _.userId)) { //当前用户是抄送人
            updateCache('audit', true);
          }
          this.startAuditList = [...result.data];
          let fieldContralList = data.fieldContralList;
          result.data.length == 1 ? _this.fieldContralList(fieldContralList) : _this.carbonFieldContralList(result.data);//抄送人可见字段;
          ...
          if(flowNodeItemKeys){
            setTimeout(() => {
              ...
              if(this.flow.isover != 2) return false;
              _this.$store.commit('updateItemKeysSelected', {index: 0, data: users});
            }, 0);
          }
      ...
      //控制抄送人可见字段
      carbonFieldContralList(data){
        var carbonFieldList = []
        for(let i =0;i<data.length;i++){
          carbonFieldList = carbonFieldList.concat(data[i].fieldContralList)
        }
        var map = {};
        carbonFieldList.forEach((item)=>{
          if(!map[item.fieldId]){
            map[item.fieldId] = item.isRead?true:false;
          }
        })
      ...
      share(shareVO){
        wx.onMenuShareTimeline({
          ...
          link: shareVO.shareUrl.split("#")[0]+"#"+shareVO.shareUrl.split("#")[1], // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
        ...
        wx.onMenuShareAppMessage({
          title: shareVO.shareTitle, // 分享标题
          desc: shareVO.shareContent.replace(/<[^>]+>/g,""), // 分享描述
          link: shareVO.shareUrl.split("#")[0]+"#"+shareVO.shareUrl.split("#")[1], // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
          imgUrl: shareVO.shareImg, // 分享图标
...
// 用户点击了分享后执行的回调函数
        ...
        wx.onMenuShareQQ({
          title: shareVO.shareTitle, // 分享标题
          desc: shareVO.shareContent.replace(/<[^>]+>/g,""), // 分享描述
          link: shareVO.shareUrl.split("#")[0]+"#"+shareVO.shareUrl.split("#")[1], // 分享链接
          imgUrl: shareVO.shareImg, // 分享图标
          success: function () {
// 用户确认分享后执行的回调函数
          },
...
// 用户取消分享后执行的回调函数
      ...
      initForm() {
        ...
        detailGetFroms(id).then(function (result) {
          _.hideLoading()
          if (result.code == "0") {
            //如果是普通单&&负责人是当前用户返回刷新代办数据
            if(result.data.controlPO.isTask == '0' && result.data.inchargesList.some(item => item.userId == _.userId)) {
              updateCache('audit', true);
            }
            //兼容旧的字表的数据
            ...
            _this.result = result;
            //数据联动
            setLinkage(result.data.fields, _this.$store);
            ...
            _this.flow.nodeId = result.data.detail.currentNodeId;
            ...
            if(result.data.controlPO && result.data.controlPO.isFreeFlow != '1'){//非自由流程
              _this.getNodeListInfo({'refId':_this.id,"agentCode":_this.agent,"nodeId":_this.flow.nodeId,'callback': function (data){
                if (data.length>0){
                  _this.flow.nodeId = data[0].nodeId
                } else {
                  _this.flow.nodeId = result.data.detail?  result.data.detail.currentNodeId : ''
                }
...
//				_this.detailPicList.push(...result.data.mediaList)
        ...
        this.showCommentList(result.data.comments, result.data.hasMore,result.data.commentCount);
        ...
        document.title  = _this.formTitle = result.data.instanceTitle;
      ...
      validations() {
        ...
        if(this.controlPO.isFreeFlow == 1){
          flag = false;
          return form_validation(schema,flag);
        }
        return form_validation(schema,flag,true);
      },
      getFormData() {
      ...
      printDetail() {
        if(isVipSilver(interfaceCode.INTERFACE_CODE_FORM)){
          ...
          if (this.isWhiteCorpId && isHasPrintTemplate == "2") {
            this.$router.push({name: 'customPrint', query: {'workOrderId': foreignid}});
          } else {
      ...
      goCopyForm() {
        this.$router.push({name: 'add', query: {id: this.id,type:'copy'}})
      },
      goEditForm() {
      ...
      async saveChange(saveType){
        var isSave = true;
        if(!this.showSave ) return isSave;
        ...
        if (this.flow.isBranchFlow) {
          queryData.currentNodeId = this.flow.nodeId;
        }
        if (saveType){
          _.showLoading('保存中...');
        }else {
        ...
        await updateFormByToUser(queryData).then(res => {
          _.hideLoading();
          if(res.code == '0'){
            _this.showSave = false;
            _this.formTitle = _this.newFormTitle || _this.formTitle;
            _this.updateTitle = false;
            if (saveType) {
              setReadonlyEdit(_this.$store, true);
