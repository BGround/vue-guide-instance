<template>
  <div class="qwui-search_wrap qwui-full_width"  :class="{'qwui-no_mask':searchinput.ridMask}">
...
//  搜索框
    ...
    created:function(){
      let keyWordCache = JSON.parse(sessionStorage.getItem('searchValue'))
      if(keyWordCache){
        this.getKeyWord = keyWordCache
        this.search()
      }
      let initIndex,
    ...
    // 销毁进入详情页面时缓存搜索数据
    beforeDestroy() {
      sessionStorage.setItem('searchValue',JSON.stringify(this.getKeyWord))
    },
    methods:{
      //点击显示morefunction
      showChooseList(){
