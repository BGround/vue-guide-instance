  ...
  methods: {
      ...
      if(item.userId){
        if(item.chargePersonList.length>0){
          ...
          getClientDetail(params,(data) => {
            if(data.code==='0'){
              //有查阅权限，跳到客户详情页面
              this.$router.push({path:"ClientDetails/ClientContent",query:{id:item.userId}})
            }else{
          ...
          //没有负责人，是公海客户，判断有无查阅权限
              ...
              if(item.poolId == 0 ){
                //默认公海，都有查阅权限
                this.$router.push({path:"PoolDetails/PoolContent",query:{id:item.userId}})
              }else{
                //非默认公海，通过调用接口判断是否有权限
                let params={
                  poolId:item.poolId,
    ...
    showContactDetail:function(item){
      ...
      getContactDetail(params,(data) => {
        if(data.code==='0'){
          //有查阅权限，跳到联系人详情页面
          this.$router.push({path:"ContactDetails/ContactContent",query:{id:item.userId}})
        }else{
          if(item.chargePersonList.length>0){
