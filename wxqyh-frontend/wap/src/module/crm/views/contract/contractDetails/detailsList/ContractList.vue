...
<script>
  ...
  import {getDetailContractList} from '../../../../api/contract/getData.js';
  export default {
    components:{
      crm_list,
      loadMore,
    ...
    methods:{
      ...
      getList:function(){
        ...
        getDetailContractList(opt,(result) => {
      ...
      loadMore:function(el){
        ...
        this.lock_roll = this.$route.name == 'contractList' && this.$route.path != '/contractList'? true: false;
      ...
      getListId:function(id){
        ...
        this.$router.push({path:'/ContractContent',query:{id:id}});
      },


      getType:function(){
        this.url=this.$router.history.current.name;
        if(this.url=="contractList"){
          this.noRecord.text="该客户暂无合同";
        }
      },

      //  新增合同
      addContract:function(){
        this.$router.push({path:'/contractOperate',query:{clientId:this.clientId}});
      }
    }
  }
</script>
