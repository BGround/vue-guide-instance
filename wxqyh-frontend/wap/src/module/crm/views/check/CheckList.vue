<template>
  <div class="qwui-check_page" :class="{'page_no_record':showRecord}">
    <!--搜索框-->
    <div>
      <search_box :keyWord="searchWord" :searchinput="searchBar" @listenToSearchBox="searchKeywordClick" :placeholder="placeholder"></search_box>
    </div>
    ...
    <nav_tabs :defaultActiveKey="my.activeKey"
      ...
      <client_check_list :loadmore="loadMore" :searchKey="searchWord"></client_check_list>
      <!--加载状态-->
    ...
    <!--无记录-->
    <no_record v-show="showRecord" :setTop="true" :hasRecord="recordData">
      <ul slot="tip" class="qwui-check_tip_msg" v-if="!this.isTabShow">
        <li>客户、联系人数据查重</li>
        <li>个人相关的客户、联系人快捷搜索</li>
        <li>与客户、联系人一键会话联系</li>
      </ul>
    </no_record>
    <flow_button v-if="showClient && this.isTabShow" :buttondata="buttonClientConfig" class="qwui-add_btn"></flow_button>
    <flow_button v-if="showContact && this.isTabShow" :buttondata="buttonContactConfig" class="qwui-add_btn"></flow_button>
  </div>
</template>
<script>
...
import client_check_list from 'crm/views/base/client_check_list';
import flow_button from '@/components/button/flow_button';
import checkData from './data'
import { searchClientRepeat,searchContactRepeat,getPhotoSetting} from 'crm/api/check/getData';
const GET_SUCCESS = '0' //获取规则列表
export default {
  components: {
    search_box,
    nav_tabs,
  ...
  methods: {
    getPhotoSet(){
        let photoSetting = {
          ...
          getPhotoSetting(photoSetting).then( res =>{
            if(res.code === GET_SUCCESS){
              this.isAccurate = res.data.is_photograph!='0'
            } else {
                _.alert('提示',res.desc);
            }
          })
    ...
    addClient(){
      this.$router.replace({path: '/clientOperate', query:{keyWord: this.searchWord}});
    },
    addContact(){
      this.$router.replace({path: '/contactOperate', query:{keyWord: this.searchWord}});
    }
  },
}
</script>
<style lang="scss" scoped>
  ...
  .qwui-check_tip_msg{
    ...
    li:before{
      ...
      height:10px;
      border-radius:50%;
      background-color:rgb(106,162,255);
      margin-right:10px;
    }
  }
</style>
