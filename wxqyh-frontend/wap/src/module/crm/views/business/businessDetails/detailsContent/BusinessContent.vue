<template>
  <div class="qwui-details_content">
    ...
    <div class="qwui-pageNumber">
      <div class="qwui-prev qwui-prevBusiness" @click="nextBusiness('pre')">上一个商机</div>
      <div class="qwui-next qwui-nextBusiness" @click="nextBusiness('next')">下一个商机</div>
    </div>
    ...
    <!--编辑按钮-->
    <div class="qwui-business_editBtn" @click="goEdit" v-if="showEditBtn">
      <button>编辑</button>
    </div>
...
<script>
  ...
  import {getBusinessDetail,nextBusiness} from '../../../../api/business/getData.js';
    ...
    methods:{
        ...
        getBusinessDetail(obj,(result) =>{
          ...
          for(let i=0;i<this.basicList.length;i++){
            for(let j=0;j<beforeClientData.length;j++){
              if(this.basicList[i].optionName===beforeClientData[j].optionName || (this.basicList[i].optionName == '内容' && beforeClientData[j].optionName == '商机内容')){
          ...
          let contactArr=[],businessArr=[],competitorArr=[];
          ...
          this.centerNav[0].value=contactArr;
          ...
          this.communicationNav[1].value=this.detailsData.estimatedSale == '0.0' ? '' : this.detailsData.estimatedSale;
      ...
      nextBusiness:function(type){
        ...
        nextBusiness(obj,(result) => {
          ...
          this.$router.push({path:path+'/BusinessContent',query:{id:result.businessId,size:10,isPartner:""}});
          this.getDetails();
        })
      },
      ...
      goEdit:function(){
        let id=this.$router.history.current.query.id;
        this.$router.push({path:'/businessOperate',query:{businessId:id}});
      },
