  ...
  <div class="qwui-user_detail_mask" v-show="showDialog">
    ...
    <div class="qwui-user_detail" v-show="!isAllInfo">
      ...
      <div class="qwui-user_detail_info" >
        ...
        <div class="name">
          <span :class="filterSex" >{{person.personName}}</span>
        </div>
        <div>
...
//          personName :{fieldName:"姓名",name:"personName", text:null},
    ...
    beforeDestroy(){
      ...
      EventBus.$off("showPersonUtil",this.showPersonMessage);
      EventBus.$off("atThisPersonUtil",this.showUserMessage);
    },
    watch:{
    ...
    computed:{
      ...
      filterSex(){
        ...
        return this.person.sex? obj[this.person.sex] : ''
      }
    },
    methods:{
      initPersonState(){
      ...
      showUserMessage(data,isUseAt){ //如果不需要@，就传true过来
        this.isUseAt = isUseAt? false : true;
          ...
          success: function (result) {
            ...
            if(result.code=="0"){
              ...
              if(userInfo){
                ...
                //请求常用联系人数据
                if(userInfo.userId!=_.userId){
                  self.getOpenClick(userInfo.userId);
                }else{
                  self.initBtn()
                }
              ...
              self.person={
                headPic:data.headPic? data.headPic:'',//头像
