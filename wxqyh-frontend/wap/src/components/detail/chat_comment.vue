  ...
  <div class="qwui-chat_comment_box" :class="{active:isBlur}">
    ...
    <div class="qwui-chat_comment_inner">
      ...
      <div class="qwui-chat_comment_input">
        <textarea class="qwui-textarea" cols="30" rows="1" v-model="messageValue" ref="input" @input="updateValue($event.target.value)" @click="textareaClick($event.target)" v-focus="isAutoFocus"></textarea>
      </div>
...
<script>
  ...
  export default {
    name: 'chatComment',
    props:['listcomment','showCrmChatCommentBox',"agent"],
    data(){
      return{
        dataBase:dataBase,
        isBlur:false, //是否聚焦
    ...
    beforeDestroy(){
      EventBus.$off("chatSendUtil",this.getChatComment);   //清除@人
    },
    watch:{
      showCrmChatCommentBox(){ //监听crm客户详情传入的showCrmChatCommentBox的变化
        if(this.showCrmChatCommentBox == true){
          this.isBlur=true;
          this.isChatBtnShow=true;
          this.isAutoFocus=true; //自动聚焦
        }
      }
    },
    methods:{
      ...
      closeChatDialog(){  //关闭置顶聊天窗口
        ...
        this.$emit("closeCrmChatCommentBox");
      },
