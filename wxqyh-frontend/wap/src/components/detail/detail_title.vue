  ...
  <div class="qwui-detail_header">
    ...
    <slot name="section">
      <div class="qwui-detail_header_time">
        ...
        <span class="qwui-detail_header_creator" @click.stop.prevent="showPersonDetail(titledata.creator)">{{$t(titledata.personName)}}</span>
        ...
        <a v-if="printTag" @click="printClick">{{configDes.prints}}</a>
      </div>
    </slot>
    ...
    <slot name="content">
      <div class="qwui-detail_instruct">
        ...
        <div class="qwui-detail_instruct_images" v-if="isPicList">
          <img :src="dealImagesUrl(item.picPath)"
               v-for="(item,index) in titledata.picList" @click="previewImage(item.picPath, titledata.picList)">
        </div>
        <!--   文件   -->
        ...
        <div class="qwui-detail_score" v-if="isScoreList">
          ...
          <div class="star" v-for="(item,index) in titledata.scoreList" :key="index">
            <span class="name">{{item.personName}}</span>
            <div class="qwui-score_box_select">
              <span class="qwui-score_star" :class="{actived:start>=item.score}"  v-for="(n,start) in 10" :key="start"></span>
            </div>
          </div>
        </div>
        <!--其他内容-->
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      checkURL(obj){
        if(this.agent == 'topic'||this.agent == 'activity'){
          this.titleContent=this.titledata.isUeditor=='1'?_.checkURL(obj, false,true):_.checkURL(obj, false,false);
          return ;
        }
        this.titleContent=_.checkURL(obj, false);
      },
      imgsClickFunc(){
        var imgs = this.$refs.instructWord.getElementsByTagName('img');
      ...
      previewImage(picPath, list){
        if (!picPath.includes('http')) {
          picPath = _.compressURL + picPath;
        }
        const _list = list.map(item => {
          if(!item.picPath.includes('http')) {
            item.picPath = _.compressURL + item.picPath;
          }
          return item;
        })
        this.$emit('on-preview',picPath, _list);
      }
    },
    components:{
      applyStatus,
