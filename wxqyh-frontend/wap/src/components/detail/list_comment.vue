          ...
          <li v-for="(item,index) in listcomment.comments" :id="item.commentId">
            ...
            <div class="qwui-comment_list_fr">
              ...
              <p v-if="item.type=='2' && item.content.indexOf('已被管理员')!=0">
                <img class="qwui-comment_list_img previewer-demo-img-comment"
                     @click="preview(item.content)"
                     :src="imgCompressURL(item.content)"/>
              </p>

              <p class="qwui-type" v-else-if="idAddLabelType(item)">
                <span :class="dealLabelType(item.commentStatus).className">{{dealLabelType(item.commentStatus).type}}</span>
                {{item.content}}
              </p>
              ...
              <span class="qwui-comment_list_delete" v-if="showDeleteComment(item.time || item.createTime,item)" @click="deleteCommentClick(index,item.commentId || item.id)">{{$t('i18n.deleteOne')}}</span>
            </div>
          </li>
        </ul>
    </div>
...
<script>
  ...
  import avator from '@/assets/images/ic_sys.png';
  export default {
    name: 'listComment',
    props: {
      agent:String,
    ...
    data(){
      return{
        ...
        httpContent:"",//评论链接
        page: 1,
        infoTipAvator:avator,//系统消息默认头像
        systemMessPersonName:"系统消息",
      }
    },
    created:function () {
    ...
    methods:{
      ...
      preview (src) {
        if ((/^http:\/\/.*$/.test(src)) || (/^https:\/\/.*$/.test(src))) {
          ...
          src = _.compressURL + src
        }
        EventBus.$emit("previewImages",src.replace('/compress/','/'),this.list);
      },
      //处理已阅、已修改\网页链接
      ...
      dealLabelType:function(commentStatus){
        if(this.agent == "crm"){
          switch(commentStatus){
            case "3": return Object({className:'qwui-crm_infoType qwui-crm_infoType_green',type:"分配"});break;
            case "4": return Object({className:'qwui-crm_infoType qwui-crm_infoType_blue',type:"领取"});break;
            case "5": return Object({className:'qwui-crm_infoType qwui-crm_infoType_red',type:"退回"});break;
            case "6": return Object({className:'qwui-crm_infoType qwui-crm_infoType_red',type:"回收"});break;
            case "7": return Object({className:'qwui-crm_infoType qwui-crm_infoType_blue',type:"转移"});break;
          }
        }
      },
      ...
      showDeleteComment:function (time,item) {  //是否显示删除评论按钮
        ...
        if ((time == this.$t("i18n.justNow") || (time.indexOf(this.$t("i18n.minsAgo")) > 0 && parseInt(time.split(this.$t("i18n.justNow"))[0]) < 30)) && creator == _.userId) {
          if ((item.commentStatus !== undefined && (item.commentStatus=='9' || item.commentStatus=='0')) || (item.status !== undefined && item.status=='0') || item.commentStatus === null) {
            return true;
          }
        }
        return false;
        ...
        if(self.listcomment.getMore){ //如果加载更多评论接口和初始化接口不一样
          ...
          if(self.listcomment.getMore.type === 'partyConstruction'){
            ...
            if (self.listcomment.comments[self.listcomment.comments.length - 1]) {
              if(this.agent == 'crm'){
                if(self.listcomment.comments[self.listcomment.comments.length - 1].commentId){
                  self.listcomment.getMore.data.lastCommentId = self.listcomment.comments[self.listcomment.comments.length - 1].commentId;
                }
                if(self.listcomment.comments[self.listcomment.comments.length - 1].id){
                  self.listcomment.getMore.data.lastCommentId = self.listcomment.comments[self.listcomment.comments.length - 1].id;
                }
              }else{
                self.listcomment.getMore.data.lastCommentId = self.listcomment.comments[self.listcomment.comments.length - 1].commentId;
              }
            ...
            success: function (result) {
              if(result.code=="0"){
                ...
                if(result.data.comments){
                  if(self.agent == 'crm'){
                    result.data.comments.forEach( item =>{ //给crm详情评论加上type==2字段，兼容评论内容为图片时的显示
                      if(item.commentStatus == 0 && self.checkPicURL(item.content)){
                        let list=Object.assign({},item,{type:2});
                        self.listcomment.comments.push(list);
                      }else{
                        let list=item;
                        self.listcomment.comments.push(list);
                      }
...
.qwui-crm_infoType_blue{
  color: #458be9;
  border: 1px solid #458be9;
}
.qwui-crm_infoType_red{
  color: #e94f4f;
  border: 1px solid #e94f4f;
}
.qwui-crm_infoType_green{
  color: #1aad19;
