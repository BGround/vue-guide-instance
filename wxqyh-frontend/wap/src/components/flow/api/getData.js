...
audit_flow_api.prototype={
  ...
  initCheck(config){
    ...
    this.initData.refId = config.id?config.id:"";
    this.initFlowData={
      ...
      if(result.code==='0'){
        ...
        if(nextNodeList && nextNodeList.length>0) {
          ...
          if (nextNodeList.length==1 && nextNodeList[0].nodeStatus=="2") {
            ...
            //新建单分支下一节点是并行开始、详情有流转，且默认的下一节点是并行结束
            if(self.initFlowData.isBranchNode && nextNodeList[0].nextNodeList.length){
    ...
    if(data && callBack){
      ...
      axios.post(_.baseURL+'/portal/flow/flowParallel/ajaxGetFlowAuditUser.do',{
        ...
        if(result.code==='0'){
          ...
          //表单字段
          if(result.data.firstFieldControlList && result.data.firstFieldControlList.length>0){
				    self.initFlowData.firstFieldControlList.push(...result.data.firstFieldControlList)
		      }
          //添加可自由选择的处理人
      ...
      axios.post(_.baseURL + '/portal/flow/flowParallel/getParallelFlowSerial.do', {
        ...
        if (result.code == '0') {
          if (result.data.auditSerial.length > 0) {
            ...
            flowSerial.currentTime = result.data.currentTime;
