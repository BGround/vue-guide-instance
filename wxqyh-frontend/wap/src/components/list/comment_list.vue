          ...
          <li v-for="(item,index) in commentList" :key="item.commentId" class="qwui-commentList_list">
            ...
            <div class="qwui-commentList_title">
              ...
              <div v-if="item.type=='2'" class="qwui-commentList_content">
                ...
                <img v-else class="qwui-commentList_img" :src="imgCompressURL(item.content)" @click="preview"/>
              </div>
              <div v-else class="qwui-commentList_content" :class="dealListType(item)">
                <p v-if="dealListContentStatus(item)"
                   class="qwui-content_word"
                   :class="{'qwui-content_address':item.type == 4}"
                   v-html="filterTextContent(item.content)"
                   @click="sendAddress($event,item)"></p>
                <img v-if="checkImageURL(item.content)" class="qwui-commentList_img"  :src="filterImageURL(item.content)" @click="preview"/>
              </div>
              <p v-if="item.entrustUserName" class="qwui-commentList_entrustUserName">{{item.entrustUserName}}委托</p>
              <span class="qwui-commentList_delete" v-if="dealTimeType(item)" @click="deleteThisComment(item,index)">删除</span>
              <span class="qwui-commentList_pass" v-if="dealShowPass(item)" @click="passThisComment(item,index)">通过</span>
            </div>
          </li>
        </ul>
      </div>
    ...
    methods: {
      dealEmojify(){
        this.$nextTick(function () {
          var demo=document.querySelector(".listcomment_comments")[0];
          emojify.run(demo);
        });
      },
      //处理列表图标类型显示
      dealListType(item){
        return this.dealListIcon(item)
      },
      //处理文本内容，剔除URL
      ...
      //判断是文本内容
      dealListContentStatus(item){
        return this.dealListContent(item)
      },
      //判断是不是有图片
      checkImageURL(str){
      ...
      //通过评论方法
      passThisComment(item,index){
        this.$emit("pass",item,index);
      },
      ...
      previewPerson(user){                                        //查看个人信息
        const userId=user.creator||user.userId||user.createPerson;
        if(userId && user.personName !== '匿名'){
          this.$emit("atThisPersonUtil",user);
        }
      },
      preview(event){                                             //预览图片
        EventBus.$emit("previewImages",event.target.currentSrc.replace(/compress\//,''), this.list);
      },
      checkImagesUrl(){                                          //检出评论的全部图片src
	   ...
	   sendAddress({target},item){
      	if(item.type == 4 &&target.localName != 'i'){
					var data = {
...
<style scoped>
  ...
  .qwui-commentList_entrustUserName{
    margin-top: 10px;
    color: #B2B9C8;
  }
