  ...
  <div>
    <div class="qwui-title_detaildata">
     <div class="qwui-title_box">
       ...
       <div class="please_select_comment" v-if="templateObj.templateType">
       ...
       <div class="select_bg_scale"></div>
          ...
          <div class="qwui-tape_box">
            ...
            <!--  手机端样式  -->
            <div v-else>
              <textarea class="item_content" :class="classObject" v-model="templateObj.content" :maxLength="contentMaxLength"
...
export default {
    ...
    return {
        ...
        config: {
          ...
          toolbars:[[ 'fullscreen', 'source', '|', 'undo', 'redo', '|', 'bold', 'italic', 'underline','forecolor', '|',  'insertorderedlist',
            'insertunorderedlist', 'fontsize', 'justifyleft', 'justifycenter', 'justifyright', 'justifyjustify', 'insertimage','inserttable']]
        }
    }
  },
  updated: function () { //进入组件时，更新内容文本域状态
    if(this.freeTextareaHeight&&this.$refs.input) {
      this.resetTextarea(this.$refs.input);
    }
  ...
  mounted () {
    if(this.freeTextareaHeight&&this.$refs.input) {
      this.box = this.$refs.input;
      this.resetTextarea(this.box);
  ...
  computed:{
    ...
    classObject: function () {//文本域css类
      return {
        'error_tip': this.contentErrorTip,
        'maxHeight': !this.freeTextareaHeight
      }
    }
  },
  watch:{
    ...
    'templateObj.lastTaskTemplate':function () {// 获取上次的任务类型
      if(this.templateObj.lastTaskTemplate){
        this.templateObj.templateTypeId = this.templateObj.lastTaskTemplate;
        this.setTitle();
      }
    }
  },
  methods:{
    resetTextarea:function(element){  //文本域高度
      ...
      if(this.templateObj.templateTypeId!=""){
        ...
        for (var i = 0; i < templateList.length; i++) {
          if (this.templateObj.templateTypeId== templateList[i].id) {
            ...
            data=templateList[i];
            if (!this.templateObj.customAlert){
