  ...
  <div class="qwui-title_file_add">
    ...
    <div class="upload_file_drag" v-if="dragDrop && !isWeChatApp">
	  ...
	  <input type="file" ref="inputFile" accept="application/msexcel"
			 @change="mediaFileUpdate" class="upload_file_drag_input" multiple>
	</div>
	<div v-else class="qwui-user_select_addpic">
	  <input type="file" ref="inputFile" @change="mediaFileUpdate" class="upload_file_input" multiple>
    </div>
	<!-- 上传媒体文件（手机端页面）引入  end -->
  </div>
</template>
...
export default {
  ...
  methods:{
	mediaFileUpdate: function(even){
	  updateFile(even,this.agent,this.updateListFun,this.isOpen,this.orderId);
	},
	updateListFun(data){
	  if(data.mediaInfo){
