        ...
        <div class="qwui-setting_inner_f_item qwui-flexbox" v-if="isShow && dataDetailMsg.levelData.show">
            ...
            <div class="qwui-flexItem">
              <select v-model="priority" class="qwui-flexItem qwui-setting_item_select " >
                <option  v-for='(data, index) in dataDetailMsg.levelData.data' :value="data.id" :key="index" >{{$t(data.level)}}</option>
              </select>
            </div>
        </div>
        <div v-if="isShow">
          <div class="qwui-setting_inner_f_item" v-for="(item, index) in dataDetailMsg.switchControl" :key="index" v-show="typeof(item.show)=='boolean'?item.show:true">
