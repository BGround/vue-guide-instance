<template>
  <div class="qwui-user-select-content">
    <div v-for="(item,index) in List" :key="index" v-if="index < rowNum || openMore"
      @click="previewPerson(item)" class="qwui-user_select_view" :class="isOrderAudit? 'isOrderAuditWidth':''">
      <div class="title-flex">
        ...
        <i class="isOrderAuditImg" v-if="isOrderAudit && (isEdit || index != List.length-1)"><img src="~assets/images/orderFreeFlow.svg" alt=""></i>
      </div>
    </div>
  ...
  export default {
    ...
    computed:{
      ...
      previewMore(){
        if(this.List){
          let len = '';
          if(!this.isOrderAudit) {
            len = this.isEdit ? 15 + 71: 15;
            this.rowNum = parseInt((this.getClientWidth() - len)/71);
          }else {
            len = this.isEdit ? 15 + 87: 15;
            this.rowNum = parseInt((this.getClientWidth() - len)/87);
          }
