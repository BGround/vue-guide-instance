        ...
        <div class="qwui-dialog_content_box" v-for="(obj,index) in dialogconfig.content">
            ...
            <div v-if="obj.type=='dateTime'">
                ...
                <div class="qwui-dialog_secondary_content">
                    <datetime v-model="dateValueList[index].value" @on-change="onChangeTime(index)" format="YYYY-MM-DD HH:mm" year-row="{value}年" month-row="{value}月" day-row="{value}日" hour-row="{value}点" minute-row="{value}分" :confirm-text="$t('i18n.sure')" :cancel-text="$t('i18n.cancel')"></datetime>
                </div>
            </div>
            <!--选人-->
            ...
            <div class="qwui-dialog_textarea" v-if="obj.type=='comment'" :class="{'active':obj.error && obj.error!=''}">
                ...
                <p class="qwui-dialog_textarea_counter">{{messageNumList[index]}}/{{maxLengthList[index]}}</p>
            </div>
            <!--错误提示、每个列表有一个错误标签提示-->
...
export default {
    ...
    methods: {
        ...
        selectRaidoClick(value, index) {
            ...
            if (value && name) {
                this.radioValueList.splice(index, 1, {
                    name: name,
                    value: value
                });
            }
        ...
        setRateValue: function (index) {
            ...
            if (!isNaN(value) && name) {
                this.rateValueList.splice(index, 1, {
                    name: name,
                    value: value
                });
            }
        ...
        scoreIconClick(num, score, index, start) {
            ...
            if (num > 0 && num <= 10 && score > 0 && score <= 10) {
                this.scoreValueList.splice(index, 1, {
                    name: name,
                    starIndex: start,
                    value: val * score
                });
        ...
        countDateTime(index) {
            if (Object.prototype.toString.call(this.dialogconfig.content[index].dateTime) === "[object Object]") {
                ...
                if (value) {
                    this.dateValueList.splice(index, 1, {
                        value: value.substring(0, 16)
                    });
                } else {
                    this.dateValueList.splice(index, 1, {
                        value: _.getCurrentTime()
                    });
                }
            }
        },
        ...
        countCommentValue(index) {
            ...
            if (this.dialogconfig.content[index].comment.value || this.dialogconfig.content[index].comment.value === 0) {
                ...
                if ((this.maxLengthList[index] - this.dialogconfig.content[index].comment.value.length) >= 0) {
                    this.messageNumList[index] = this.dialogconfig.content[index].comment.value.length
                } else {
                    this.messageNumList[index] = this.maxLengthList[index]
                }
            } else {
                this.commentValueList[index].value = "";
                this.messageNumList[index] = 0
            }
        },
        //显示错误提示
        showErrorTip() {
        ...
        dialogBtnClickTypeTwo: function (msg) {
            ...
            if (msg.callBack) {
                ...
                if (this.dialogconfig.type == "dialog2") {
                    ...
                    if (hasUserSelect) {
                        newCommentValueList.splice(0, 0, {
                            name: this.dataBase.selectConfig.selectList[this.dataBase.selectConfig.signIndex].userName,
                            value: this.userId
                        });
                        this.userId = "";
                    }
        ...
        updateValue: function ($el, index) {
            ...
            if ((this.maxLengthList[index] - value.length) < 0) {
                ...
                this.messageNumList.splice(index, 1, value.length);
            }
...
.qwui-dialog_slider_box .range-min,
.qwui-dialog_slider_box .range-max {
    display: none
}
