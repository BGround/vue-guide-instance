//路径：wap\node_modules\vux\src\components\datetime\datetimepicker.js
      ...
      if (type === 'year') {
        ...
        if(type === 'day'){// 添加星期begin
          ...
          if(isIos){
            ...
            name = parseRow(config[type + 'Row'], val) + " 星期"+['日','一','二','三','四','五','六'][week];
          }else{
            let week = new Date(jim.currentYear+'-'+jim.currentMonth+'-'+val).getDay();
            name = parseRow(config[type + 'Row'], val) + " 星期"+['日','一','二','三','四','五','六'][week];
          }
        ...
        // console.log(WEEK.currentYear,WEEK.currentMonth,type,val,name)
      }
      data.push({
        name: name,
        value: i
