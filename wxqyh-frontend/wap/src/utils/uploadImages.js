...
export function wx_chooseImage(agent,callBack,count,type,orderId){		//微信端图片上传公用方法
  if(!type){
    type=['album', 'camera'];
  }
  if(!count){
        ...
        wx.uploadImage({
          ...
          success: function (res) {
            ...
            if(serverId.length==localId.length){
              newimageupload(serverId,agent,callBack,orderId);
            }
          },
          fail: function (res) {
            _.hideLoading();
...
function newimageupload(serverId,agent,callBack,orderId){
    let serverIds="";
...
export function qw_uploadImages(event,agent,callBack,orderId){	           //PC端图片上传公用方法
  ...
  for(let i=0;i<obj.files.length;i++){
    let imageName = obj.files[i].name
    if(imageName.lastIndexOf('.')>-1){
      const type = imageName.substring(imageName.lastIndexOf('.') + 1, imageName.length).toLowerCase();
  ...
  if (obj.files && obj.files[0]) {
    ...
    for(let j=0;j<file.length;j++){
      let img = new Image();
      img.src = URL.createObjectURL(file[j]);
      img.onload = function () {
        let that = this;
        let quality=1;
        let maxWidth=2560;
