...
axios.defaults.transformRequest=[function (data) {
  ...
  if(window.document.location.href.indexOf("vpModule.html") != "-1" && data.indexOf("requestSource") == "-1") {
...
function getLoginMessage(resolve){
  ...
  if(window.location.href.indexOf("/open")!=-1){//外部页面判断处理
    ...
    axios({
      method: 'post',
      url: baseURL + '/portal/wxqyhLoginCtrl/getPortalUserInfoForMgr.do?corp_id=' + corp_id + '&agentCode=' + agentCode
    })
      .then(function (response) {
        if (response.code == "0") {
