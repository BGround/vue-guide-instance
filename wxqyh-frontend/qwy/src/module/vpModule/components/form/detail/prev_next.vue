...
<script>
	import { changePage } from 'vpModule/api/form/list.js'
	export default {
		data () {
			return {
				pageData:{},
		...
		methods:{
			...
			goChange(page) {
				...
				changePage(this.pageData).then((res) => {
					_this.curPage = _this.pageData.paramPagerNum;
          if(_this.countPage != res.data.paramPageCount){
            _this.countPage = res.data.paramPageCount;
            _this.pageData.paramPageCount = res.data.paramPageCount;
            sessionStorage.setItem('pageData',_this.pageData)
          }
					_.hideLoading()
					if(res.code != "0"){
