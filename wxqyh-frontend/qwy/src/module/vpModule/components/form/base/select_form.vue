        ...
        <div class="qwui-user-list">
          ...
          <div v-for="item in list" class="qwui-user_item" :key="item.id">
            <div class="qwui-user_item_inner" @click="selectThis(item)">
              ...
              <!--遍历显示的数据-->
              <div v-if="onlyTitle" class="only-title " v-html="formatData(item, refCfg.main_field)"></div>
              <div v-else class="title description_title flexItem">
...
<script>
	...
	import search_bar from 'vpCommon/base/search_box.vue';
	import loadMore from './load_more.vue'
	import { selectFormdata } from 'vpModule/api/form/list'
  ...
  export default {
    ...
    computed:{
      ...
      onlyTitle() {
        return this.refCfg.show_fields.length == 0 && this.typeMap[this.refCfg.main_field] != 'ImageField'
      }
    },
    created(){
    ...
    methods: {
      ...
      comfirm(){
        ...
        var currentData = {}
        this.combineList.forEach(function (x) {
          if(x._id === currentId) {
            currentData = x
          }
        });
        ...
        if(min && len < min){
          _.alert('提示','最少输入'+ min +'项');return;
        }else if(max && len > max){
          _.alert('提示','最多输入'+ max +'项');return;
        }
      ...
      formatData(choiceMap,choiceItem){
        ...
        for(let field of fieldMap ){
          if(field._id === choiceItem){
            ...
            //地理定位
            else if (field._type === 'GeoField') {
              var value;
              try {
                value = JSON.parse(choiceMap[choiceItem]).value
              } catch (e) {
                value = choiceMap[choiceItem] || "";
              }
              return value;
            }
            //图片
            else if(field._type === 'ImageField'){
              var imgs = choiceMap[choiceItem]?choiceMap[choiceItem].split(','):[];
              var pic = [];
              imgs.forEach(img => {
