...
<script>
  ...
  import {cloneDeep,getChildDefault} from 'vpModule/js/form/getData'
  export default {
    components:{
      childItem
    },
    ...
    methods:{
      ...
      resetChildView(){
        ...
        if(this.defaultValue){
          value.forEach(item =>{
            Object.keys(_this.defaultValue).forEach(key => {
              if(!item[key]) item[key] = _this.defaultValue[key];
            })
          })
        }
      ...
      selectData(){
        ...
        dataBase.selectForm.confirm = function(data){
          ...
          //主子段的类型是地理定位则取value
          var mainFieldType = ''
          _this.field.ref_cfg.show_fields_array.forEach(item => {
            if(_this.field.ref_cfg.main_field == item._id) {
              mainFieldType = item._type
            }
          })
          data.selectedList.forEach(item => {
            if(mainFieldType == "GeoField") {
              var name = "";
              try {
                name = JSON.parse(item[_this.field.ref_cfg.main_field]).value
              } catch (e) {
                name = item[_this.field.ref_cfg.main_field] || '';
              }
              item.value = item.id + "_" + name;
            } else {
      ...
      initDefaultData(){
			  if(_.isNull(this.field.children)){
			    return this.resetChildView();
        }
        var value = this.field.value = this.field.value || [];
          //设置子表单的默认值
          var param = {
          ...
          getChildDefault(param).then(function (result) {
            if (result.code == "0") {
              _this.defaultValue = result.data.default || {};
              _this.resetChildView();
            }
          });
        }
