  ...
  <div class="qwui-dialog_overlay" v-show="dialogconfig.show" @touchmove.prevent>
    ...
    <div class="qwui-dialog z-index_top" v-if="dialogconfig.type=='dialog1'">
      ...
      <div class="qwui-dialog_header" v-if="dialogconfig.title">
        ...
        <img src="../../../assets/images/btn_close.png" alt="" class="qwui-dialog_title_close" @click="closeDialog">
      </div>
      ...
      <div class="qwui-dialog_content_box" v-for="(obj,index) in dialogconfig.content">
        ...
        <!--评语-->
          <div class="qwui-dialog_textarea" v-if="obj.type=='comment'"  :class="{'active':obj.error && obj.error!=''}">
              ...
              <p class="qwui-dialog_textarea_counter" :class="{colorRed:maxLengthList[index]-messageNumList[index] == 50}">{{maxLengthList[index]-messageNumList[index]}}/{{maxLengthList[index]}}</p>
          </div>
        <!--错误提示、每个列表有一个错误标签提示-->
...
<script>
  ...
  import user_add from 'vpCommon/add/user_select_build';
  export default{
    name: "dialogMask",
    data(){
      return {
...
//        判断换行
      ...
      closeDialog() {
        if(dataBase.dialogConfig.btnConfig.close){
          dataBase.dialogConfig.btnConfig.close.callBack()
        }else {
          _.showMsgHide();
        }
      }
...
<style scoped>
  ...
  .qwui-dialog_btn_primary{
    ...
    border-radius: 2px;
    line-height: 32px;
    border: 1px solid rgba(226,117,10,1);
    color: #FFFFFF;
  }
  .qwui-dialog_btn_primary:hover {
    background: #F08088;
  }
  .qwui-header_title{
    position: relative;
