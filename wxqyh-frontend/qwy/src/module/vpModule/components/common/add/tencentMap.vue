<!--使用： 在html页面引入腾讯地图api和校准库
	      pos必传 undefined或object -->
	    <!--type类型 显示地图的类型 getPosition获取定位并显示地图 showPosition展示定位  getAddress获取定位不展示地图 其他待定-->
...
<script>
	...
	import flowButton from 'vpCommon/button/flow_button';
...
//			初始化地图
			...
			getLocation(){
				var _this = this;
				if( !this.noPos  &&this.type!='getAddress'){
              ...
              cancel:function (res) {
                _this.renderAddress (false,'用户拒绝授权获取地理位置');
                _this.buttonConfig.primaryList[0].callBack();
                _this.renderAddress (false,'用户拒绝授权获取地理位置');
              },
              fail:function (er) {
                _this.renderAddress (false,'获取定位失败');
                _this.buttonConfig.primaryList[0].callBack();
              }
						});
					}else{
