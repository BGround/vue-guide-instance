...
function form_validation(schema,itemMap) {
    //先清空提示
    ...
    for (var item of schema) {
        ...
        // if (item.validations && item.validations.indexOf('presence') != -1 && (!value &&!itemMap[item._id] &&itemMap[item._id]!=undefined)) {
      ...
      if (['CRMCustomerField', 'MEMBERCustomerField', 'DEPTCustomerField'].indexOf(item._type) !== -1) {
        ...
        //数字字段最大值最小值验证
			if(item._type == "NumberField" && String(value) && (item.range_max || item.range_min)){
