    ...
    <el-dialog
      ...
      <div class="dialog__content">
        ...
        <div class="dialog__content__options">
          <div v-for="(item) in getFilteredOptions" :key="item.id" class="products__item" :class="{active: item.erpSerNo === selectedErpSerNo}">
            <label>
              <span>{{item.dealerFullName}}</span>
              <span> <input type="radio"  v-model="selectedErpSerNo" :value="item.erpSerNo"></span>
            </label>
          </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed:{
            getFilteredOptions(){
                if(this.keyword){
                    return this.options.filter(item=>item.dealerFullName.includes(this.keyword)  || item.erpSerNo.includes(this.keyword))
                }else{
                    return this.options;
                }
        ...
        methods: {
            ...
            handleConfirm(){
                ...
                const item = this.options.find(item=>item.erpSerNo === this.selectedErpSerNo);
                if(item){
                    this.$emit('confirmERP', this.selectedErpSerNo);
            ...
            init(erpSerNo){
                this.searchServiceErp().then((options)=>{
                    const option = options.find(item=> item.erpSerNo === erpSerNo);
                    if(option){
                        this.selectedErpSerNo = option.erpSerNo || '';
                        this.$emit('confirmERP', option.erpSerNo);
