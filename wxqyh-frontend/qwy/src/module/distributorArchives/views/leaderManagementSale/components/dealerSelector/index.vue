<template>
  <div class="brand-select">

    <qwuiButton @click="handleClickOpenBtn" type="button" >&nbsp;选择ERP号&nbsp;</qwuiButton>

    <el-dialog
            :title="id?'编辑':'新增'"
            :visible.sync="isVisibleDialog"
            width="400px">
      <div class="dialog__content">

        <div class="dialog__content__search">
          <el-input v-model="keyword" size="small" @input="handleInputKeyword" placeholder="请输入内容"></el-input>
        </div>


        <div class="dialog__content__options">
          <div v-for="(item) in getFilteredOptions" :key="item.id" class="products__item" :class="{active: item.erpCarNo === selectedErpCarNo}">
            <label>
              <span>{{item.dealerFullName}}</span>
              <span> <input type="radio"  v-model="selectedErpCarNo" :value="item.erpCarNo"></span>
            </label>
          </div>
        </div>



      </div>
      <div slot="footer">
        <div class="footer">
          <qwuiButton @click="handleConfirm" type="button" >&nbsp;确定&nbsp;</qwuiButton>
        </div>
      </div>

    </el-dialog>
  </div>
</template>

<script>
    import qwuiButton from '@/components/qwuiBase/qwuiButton/qwuiButton'
    import {searchServiceErp } from "module/distributorArchives/api/getData";
    import debounce from '@/module/distributorArchives/utils/debounce'

    const fetchServiceErp = debounce(function () {
        this.filterServiceErp()
    }, 700)

    export default {
        name: "dealerSelector",
        components:{
            qwuiButton
        },
        data() {
            return {
                isVisibleDialog: false,
                keyword: '',
                selectedErpCarNo: '',
                options: []
            }
        },
        model:{
            prop: 'erpCarNo',
            event: 'confirmERP'
        },
        props: {
            erpCarNo: {
                type:String,
                default(){
                    return '';
                }
            },
            id:{
                type:String,
                default(){
                    return '';
                }
            }
        },
        computed:{
            getFilteredOptions(){
                if(this.keyword){
                    return this.options.filter(item=>item.dealerFullName.includes(this.keyword)  || item.erpCarNo.includes(this.keyword))
                }else{
                    return this.options;
                }

            },
        },
        methods: {

            handleInputKeyword(){
                fetchServiceErp.call(this);
            },

            handleClickOpenBtn(){
                this.isVisibleDialog = true;
                this.searchServiceErp()
            },

            filterServiceErp(){

            },


            searchServiceErp(){
                return searchServiceErp({
                    serviceErp: this.serviceErp,
                }).then((data)=>{
                    this.options = data || [];
                    return this.options;
                })
            },

            handleConfirm(){
                this.isVisibleDialog = false;
                const item = this.options.find(item=>item.erpCarNo === this.selectedErpCarNo);
                if(item){
                    this.$emit('confirmERP', this.selectedErpCarNo);
                    this.$emit('confirm', item);
                };
            },

            init(erpCarNo){
                this.searchServiceErp().then((options)=>{
                    const option = options.find(item=> item.erpCarNo === erpCarNo);
                    if(option){
                        this.selectedErpCarNo = option.erpCarNo || '';
                        this.$emit('confirmERP', option.erpCarNo);
                        this.$emit('confirm', option);
                    }else{
                        this.selectedErpCarNo = '';
                    }
                })
            }

        },
        created() {
            this.init(this.erpCarNo);
        },
        watch:{
            erpCarNo(value){
                this.init(value);
            }
        }
    }
</script>

<style scoped lang="scss">

  .brand-select{
    display: inline-block;

    input.btn {
      padding: 4px 20px;
    }
    a.btn {
      padding: 4px 16px;
      line-height: 22px;
    }


    input[type=radio]:checked:before {
      content: url(data:image/png;base64,);
      display: block;
    }
    input[type=radio]:before {
      content: url(data:image/png;base64,);
      display: block;
    }
    input[type=radio]:checked:disabled:before {
      content: url(data:image/png;base64,);
      display: block;
    }
    input[type=radio]:disabled:before {
      content: url(data:image/png;base64,);
      display: block;
    }
    input[type=radio],input[type=checkbox]{
      outline: 0;
      -webkit-appearance: none;
      appearance: none;
      line-height: 1;
    }


    /deep/{
      .el-dialog__footer,
      .el-dialog__header{
        background: #efefef;
        padding: 10px 20px 10px;
      }
      .el-dialog__body{
        background: rgba(239,239,239, 0.4);
        padding: 0px;
        .el-input__inner:focus{
          border: 1px solid #C31725;
        }
      }
      .el-dialog__headerbtn{
        top: 10px;
      }

      .el-radio__inner:after{
        background: #C31725;
        width: 8px;
        height: 8px;
      }
      .el-radio__inner {
        width: 20px;
        height: 20px;
      }
      .el-radio__inner:hover{
        border-color: #C31725;
      }
      .el-radio__input.is-checked{
        .el-radio__inner {
          border-color: #C31725;
          background: #ffffff;
        }
      }
      .el-radio__input.is-checked+.el-radio__label{
        border-color: #C31725;
      }

      .products__item{
        display: flex;
        justify-content: space-between;
      }

      .qwui-upload_item{
        width: 100%;
      }
    }

    .dialog__content__search{
      padding: 20px ;
      border-bottom: 1px solid #dcdfe6;
    }

    .dialog__content__options {
      height: 300px;
      overflow: auto;
    }

    .products{

    }

    .products__item{
        padding: 5px 12px 5px 20px;
        box-sizing: border-box;
        overflow: hidden;
        cursor: pointer;
      &.active {
        background-color: #eee;
      }
      label{
        width: 100%;
        display: flex;
        justify-content: space-between;
        align-items: center;
      }
    }

    .footer{
      text-align: center;
    }
  }

</style>
