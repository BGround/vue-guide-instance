...
<!--        <qwuiButton type="primary" @click="handleClickAddBtn">&nbsp;新增&nbsp;</qwuiButton>-->
    ...
    <el-dialog
      ...
      <div class="dialog__content">
        ...
        <importExcel v-if="isVisibleDialog" reportType="PRE_SERVICE_ADMIN_INFO"></importExcel>
...
<script>
    ...
    import { ajaxDeleteSaleAdmin, ajaxSearchSaleAdmin} from "../../api/getData";
        ...
        methods: {
            handleClickExportBtn() {
              if (this.pageParams.totalRows>0) {
                let params = {}
                params['searchValue.keyWord'] = this.keyWord
                params['reportType'] = 'PRE_SERVICE_ADMIN_INFO'
                // console.log('export',params)
                this._createReport(params)
              } else {
                this.$message({
                    message: '搜索结果为空，不支持导出',
                    type: 'warning'
                });
              }
            },
            ...
            handleClickRemoveBatchBtn(){
                ...
                ajaxDeleteSaleAdmin(params).then((data)=>{
                    if(data){
                        this.searchLeaders();
                    }
                })
            ...
            handleClickRemoveBtn(ids){
                ajaxDeleteSaleAdmin({'ids[]':ids}).then((data)=>{
                    if(data){
                        this.searchLeaders();
                    }
                })
            ...
            searchLeaders(){
                ...
                ajaxSearchSaleAdmin(params).then((data)=>{
                    if(data){
                        ...
                        this.tableData = pageData.map((item)=>{
                            return Object.assign({}, item, {
                                ...
                                dealerShortName: item.dealerShortName? item.dealerShortName:'-',
