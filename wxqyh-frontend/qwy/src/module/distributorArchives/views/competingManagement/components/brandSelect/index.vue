<template>
  <div class="brand-select">
    ...
    <qwuiButton @click="handleClickOpenBtn" type="button" >&nbsp;选择品牌&nbsp;</qwuiButton>
    ...
    <el-dialog
      ...
      <div class="dialog__content">
        ...
        <div class="dialog__content__options">
          <div v-for="(item) in options" :key="item.id" class="products__item" :class="{active: item.id === selectedId}">
            <label>
              <span>{{item.company}}</span>
              <span> <input type="radio"  v-model="selectedId" :value="item.id"></span>
            </label>
...
<script>
    ...
    import {planEnterprisePager} from "module/distributorArchives/api/getData";
    import debounce from '@/module/distributorArchives/utils/debounce'

    const fetchCompetingProducts = debounce(function () {
        this.searchCompetingProducts()
    }, 700)
    ...
    export default {
        ...
        methods: {
            ...
            handleInputKeyword(){
                fetchCompetingProducts.call(this);
            },
            ...
            searchCompetingProducts(){
                return planEnterprisePager({
                    page:1,
                    pageSize: 200,
                    keyWord: this.keyword
                }).then((data)=>{
                    this.options = data && data.pageData? data.pageData: [];
                    return this.options;
                })
            },
            ...
            handleConfirm(){
                ...
                const item = this.options.find(item=>item.id === this.selectedId);
                if(item){
                    this.$emit('confirm', item.company);
                };
            },

            init(name){
                this.keyword = this.name;
                this.searchCompetingProducts().then((options)=>{
                    const option = options.find(item=> item.company === name);
                    if(option){
