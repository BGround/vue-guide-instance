<template>
  <qw-select-modal class="qw-select-client"
                   title="选择客户"
                   v-model="visible"
...
import { getClientList } from "crm/api/common";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getClientList(this.params).then(res => {
        if (res.code == "0") {
    ...
    getSearchList() {
      ...
      getClientList(this.searchParams).then(res => {
        // console.log(res);
        if (res.code == "0") {
