...
<script>
import { getClientDetail } from "crm/api/client";
                    ...
                    if(basicList && basicList.length > 0) {
                        this.detailData.area2.map(detailItem => {
                            if(detailItem.key === 'area'){  // 区域特殊处理
                                if(tbCrmClientInfoPO['provinceCode'] == '其他'){
                                    this.$set(detailItem, 'value', `${tbCrmClientInfoPO['country']}`);
                                }else{
                                    this.$set(detailItem, 'value', `${tbCrmClientInfoPO['provinceCode']} ${tbCrmClientInfoPO['cityCode']} ${tbCrmClientInfoPO['areaCode']}`);

                                }
                            }
                            else if(detailItem.key === 'typeNames'){
                                this.$set(detailItem, 'value', tbCrmClientInfoPO[detailItem.key].split('|').join('，'));
                            }
                            else if(detailItem.key === 'gradeDes'){
                                let isShow = tbCrmClientInfoPO.enabledClientGrade === '1'? true : false;
                                this.$set(detailItem, 'show', isShow);
