    ...
    <div class="qwui-diary_conatiner-l">
      ...
      <div class="qwui-rule_warp">
        <ul class="qwui-rule_list">
          <li :class="{ 'active':key == renderConfig.currentIndex,'ban':item.status == 0 }" v-for="(item,key) in renderConfig.renderList" :key="key" @mouseleave="hideMore" @click="chooseItem(item.id,key)">
            <p>{{ item.itemName }}
              <i v-if="item.itemIcon" class="qwui-icon" :style="{ 'background':'url('+item.itemIcon+') no-repeat','left':'30px' }"></i>
            </p>
            <i class="qwui-a_more" @mouseenter="showMore(key)" v-if="item.operateList ? true : false"></i>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      chooseItem(id,key){
        this.currIndex = key;
        this.$emit('listenToChild',id,key)
      },
      addPopShow(){
...
<style lang="scss" scoped>
  ...
  .qwui-rule_list li:first-child{
      padding-left: 0;
  }
  .qwui-rule_list li:first-child i:first-child {
      left: 12px !important;
  }
</style>
