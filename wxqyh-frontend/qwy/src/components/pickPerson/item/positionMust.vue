  ...
  <div class="qwui-main_position clearfix">
    ...
    <ul class="qwui-position_list" ref="positionList">
      <li class="qwui-list_item clearfix"
        ...
        <span class="qwui-list_item_text">{{item}}</span>
        <label :for="item" class="qwui-position_checkbox">
          <input type="checkbox"  v-model="positionChecked" :value="item" :id="item" :disabled="isDisabled(item)">
          <span></span>
        </label>
      </li>
      <li>
        <load-more :loadmore="loadMore"
            @loadFinished="requestFinished" @showMsgFromLoadMore="searchFinished"
  ...
  export default {
    ...
    data(){
      return{
        ...
        loadMore:{ //加载更多
          url:"/post/postAction!getPositionList.action",//请求路径
    ...
    created:function () {
      this.$nextTick(()=>{
        ...
        //console.log(this.loadMore.pageConfig)
      })
