<template>
  <div class="clearfix"
    ...
    <div class="qwui-admin_title">
      <!--<div v-if="!batchShow">-->
        <!--<span>子账户名称</span>-->
        <!--<span class="batch" @click="batchShow = true">批量导入</span>-->
        <!--&lt;!&ndash;<span class="user">绑定的成员</span>&ndash;&gt;-->
      <!--</div>-->
      <!--<div v-else-->
    ...
    <div class="qwui-admin_list" v-if="admins.length > 0">
      <ul class="qwui-dept_list">
        <dealer-list :itemData="item"
          :totalData="allAdmin"
          ...
          :key="index"></dealer-list>
      </ul>

    </div>

    <div class="qwui-admin_list" style="text-align: center;" v-else>
      <span>暂无数据</span>
    </div>
  </div>
</template>
...
<script>
import { getDealerList } from '../api/getData'
import dealerList from '../checkList/dealerList.vue'
...
export default {
  ...
  data() {
    return {
      tagCheckedList: [], //批量导入界面 显示隐藏 控制
      currentIndex: 0, //批量导入界面 显示隐藏 控制
      isTagOnly: true, //批量导入界面 显示隐藏 控制
  ...
  watch: {
    ...
    checkList: function(val) {
      this.$emit('adminChecked', this.superAdmin, val)
    },
  ...
  methods: {
    isDisabledTag(){},
    searchTagPerson(currentIndex,index){
      //console.log(index)
      this.currentIndex = index;
    },
    // *初始化管理员数据
    loadData() { // /wxqyh/dealer/dealerAction!getAllDealerFullNameListBySearch.action?dqdp_csrf_token=998e4f8095d1baca
      let self = this
      getDealerList()
        .then(res => {
          //console.log(res)
          if (res.code == '0') {
            ...
            self.allAdmin = res.data.dealerVOs
           // console.log(self.superAdmin)
            res.data.dealerVOs.forEach(e => {
              self.admins.push(e)
            })
