<template>
    <li class="qwui-list_item clearfix" >
      <div class="qwui-list_item_content" @click="loadChildren(itemData)">
        <!--  有子集  -->
        <i class="qwui-see_child"
          :class="{'active':isExpand}"
          v-if="childrenList.length > 0"></i>
        <span>{{itemData.personName}}</span>
        <span class="qwui-admin_person_name"
            :style="{'right':rightPosition + 'px'}"
            v-if="itemData.tbQyUserInfoVO"
            ref="user">
            <img class="qwui-list_item_headpic"
                v-if="itemData.tbQyUserInfoVO.headPic && itemData.tbQyUserInfoVO.headPic != '0'"
                :src="itemData.tbQyUserInfoVO.headPic">
            <div v-else class="qwui-list_item_headpic default_img"></div>
            {{itemData.tbQyUserInfoVO.personName}}
        </span>
        <span v-else
            class="qwui-admin_person_name no_user">-</span>
        <label :for="itemData.userId" class="qwui-tag_checkbox" @click.stop>
          <input type="checkbox"
                v-model="checkList"
                :value="itemData"
                :id="itemData.userId"
        ...
        <ul class="qwui-list" v-if="isExpand && childrenList.length > 0">
         <admin-list  :itemData="item"
                      :totalData="totalData"
                      :isOverAdmin="isOverAdmin"
                      :defaultAdmin="defaultAdmin"
                      :deleteSpecialAdmin="deleteSpecialAdmin"
                      @adminChecked="childAdminChecked"
                      v-for="(item,index) in childrenList" :key="index"></admin-list>
        </ul>
    </li>
</template>
<script>
  ...
  export default {
    ...
    watch:{
      checkList:function (val) {
        this.$emit("adminChecked", this.itemData, val.length);
      },
