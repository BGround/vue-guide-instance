...
<script>
  ...
  import admin from './item/subAdmin.vue'
...
/*        if(this.searchMore.pageConfig.pageData && this.searchMore.pageConfig.pageData.length > 0) {
      ...
      updateValue:function (value) {
        ...
        if(value && !this.isPosition) {
          ...
          self.timer = setTimeout(function () {
            ...
            self.searchMore.url = '/task/taskMgrCtl/getUserList.do'
      ...
      //选择完成，点击确定
      chooseDone:function () {
        this.$emit('chooseDone',this.specialAdmin)
      },
      //清空已选
      clearAll(){
      ...
      batchSuccess:function (val) {
        ...
        val.forEach(function (e) {
          isFind = '';
          isFind = self.specialAdmin.find(function(item){
            return item.userId == e.userId
          });
          isFind?'':self.specialAdmin.splice(self.specialUsers.length,0,e);
        })
      ...
      adminChecked:function (val, isChecked) {
       ...
       console.log(isChecked)
        ...
        if(isChecked == 1) {
          this.specialAdmin.push(val);
        }
        if(isChecked == 0) {
          this.removeObjArr(this.specialAdmin,'userId',val.userId);
        }
        this.specialAdmin = this.delObjArr(this.specialAdmin,'userId');
        console.log(this.specialAdmin)
