      ...
      <div class="qwui-main_content">
        ...
        <div class="qwui-main_result" ref="inputChange">
          <div class="qwui-selected_input" @click.self="inputFocus = !inputFocus">
            ...
            <!--  部门已选择  -->
            <div class="qwui-dept_selected" v-for="(item,index) in deptSelected" :key="index">
              <div class="qwui-icon dept"></div>
              {{item.nodeName ? item.nodeName : item.deptName}}
              <span class="qwui-selected_close" @click="deleteDept(item,index)">×</span>
            </div>

            <!--  标签选择  -->
            <div class="qwui-inline">
              <div class="qwui-dept_selected" v-for="(item,index) in tagList" :key="index">
                <div class="qwui-icon tag"></div>
                {{item.tagName}}
                <span class="qwui-selected_close" @click="deleteTag(index)">×</span>
              </div>
            </div>

            <!--  人员选择  -->
            <div class="qwui-inline">
              <div class="qwui-dept_selected" v-for="(item,index) in specialUsers" :key="index">
                <img v-if="item.headPic && item.headPic!='0'" class="qwui-icon dept" :src="item.headPic">
                <div v-else class="qwui-icon default"></div>
                {{item.personName}}
                <span class="qwui-selected_close" @click="deleteUser(item,index)">×</span>
              </div>
            </div>

            <!--  职位选择  -->
            <div class="qwui-inline" v-for="(item,index) in specialPosition" :key="index">
              <div class="qwui-dept_selected">
                <div class="qwui-icon position"></div>
                {{item.name}}
                <span class="qwui-selected_close" @click="deletePosition(index)">×</span>
              </div>
            </div>
        ...
        <!--  选择部门、选择成员组件  -->
        <deptUser v-if="currentIndex == '部门'||currentIndex == '成员'||currentIndex == '特定人员'"
                  :isUser="isUser"
                  :loadMore = "loadMore"
                  :defaultUser="specialUsers"
                  :defaultDept="deptSelected"
                  :deleteSpecialDept="deleteSpecialDept"
                  :isOverUser="isOverUser"
                  ...
                  ref="deptUser"></deptUser>

        <!--  选择标签人员组件  -->
        <tag v-if="currentIndex == '标签'||currentIndex == '标签成员'||currentIndex == '标签人员'"
             :isTagOnly="isTagOnly"
             :loadMore = "loadMore"
             :defaultTag="tagList"
             :defaultUser="specialUsers"
             :defaultDept="deptSelected"
             :deleteSpecialUser="deleteSpecialUser"
             :deleteSpecialDept="deleteSpecialDept"
             :isOverUser="isOverUser"
             :isOverTag="isOverTag"
             :isOverDept="isOverDept"
        ...
        <!--  选择职位组件  -->
        <position v-if="currentIndex == '职位'"
               :defaultPosition="specialPosition"
               :maxPosition="maxPosition"
               @positionChecked="positionChecked"></position>

        <!--  选择管理员组件  -->
        <admin v-if="currentIndex == '管理员'"
               @adminChecked="adminChecked"
               :defaultAdmin="specialAdmin"
               :defaultUser="specialUsers"
        ...
        <!--  高级筛选组件  -->
        <senior-select v-if ="currentIndex == '高级筛选'"
                       :defaultUser="specialUsers"
                       :isOver="isOverUser"
                       :intersectionDeptIds="intersectionDeptIds"
                       :intersectionTagIds="intersectionTagIds"
                       :intersectionUserIds="intersectionUserIds"
                       @userChecked="seniorUserChecked"></senior-select>
...
<script>
  ...
  import admin from './item/admin.vue'
...
/*        if(this.searchMore.pageConfig.pageData && this.searchMore.pageConfig.pageData.length > 0) {
      ...
      updateValue:function (value) {
        if(value && this.isPosition){
          document.querySelectorAll('.qwui-result_list')[1].style.top = (this.$refs.inputChange.clientHeight + 25) + 'px'
          getPositionByKey(value).then(res => {
            this.positions = res.data.postVOs
          })
        }
        if(value && !this.isPosition) {
          document.querySelectorAll('.qwui-result_list')[0].style.top = (this.$refs.inputChange.clientHeight + 25) + 'px'
          ...
          self.timer = setTimeout(function () {
            ...
            self.searchMore.url = '/contact/selectUserMgrAction!ajaxSearch.action'
            self.searchMore.data = {
              page: 1,
              pageSize: 10,
              'searchValue.personName': value,
              intersectionDeptIds:self.intersectionDeptIds,
              intersectionTagIds:self.intersectionTagIds,
              intersectionUserIds:self.intersectionUserIds,
              agentCode:self.dataBase.agent
            };
      ...
      addToInput:function (val) {
        if(val.userId && !this.isOverUser){
          let isFind = this.specialUsers.findIndex(item => {
            return item.userId == val.userId
          })
          isFind!=-1 ? '':this.specialUsers.splice(this.specialUsers.length,0,val)
        } else if(val.nodeId && !this.isOverDept){
          let isFind = this.deptSelected.findIndex(item => {
            return item.nodeId == val.nodeId
          })
          isFind!=-1 ? '':this.deptSelected.splice(this.deptSelected.length,0,val)
        }
      ...
      searchFinished:function (val) {
        if(val.currPage && (val.currPage >= val.maxPage)){
          ...
          getDeptByKeyword(
            this.searchMore.data['searchValue.personName'],
            this.intersectionDeptIds,
            this.intersectionTagIds
          ).then(res => {
            this.searchMore.pageConfig.pageData = this.searchMore.pageConfig.pageData.concat(
              res.data.orgList
            );
          })
        }
      ...
      adminChecked:function (val, isChecked) {
        this.deleteSpecialAdmin = {}
        if(isChecked == 1){ //避免重复出发添加数据
          let isFind = this.specialAdmin.find(function(item){
            return item.userId == val.userId
          });
          if(!isFind){
            this.specialAdmin.splice(this.specialAdmin.length,0,val);
          }
        } else {
          this.specialAdmin = this.specialAdmin.filter(function(item){
            return item.userId != val.userId
          });
        }
      },
