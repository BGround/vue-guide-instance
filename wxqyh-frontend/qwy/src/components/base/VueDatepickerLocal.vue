  ...
  <div class="datepicker"
    ...
    <transition name="datepicker-anim">
      <div class="datepicker-popup"
        ...
        <template v-if="range">
          <vue-datepicker-local-calendar v-model="dates[0]"
            :left="true"></vue-datepicker-local-calendar>
          <vue-datepicker-local-calendar v-model="dates[1]"
            :right="true"></vue-datepicker-local-calendar>
        </template>
        <template v-else>
        ...
        <div v-if="showButtons"
          ...
          <button @click.stop="cancel"
            class="datepicker__button-cancel">{{this.local.cancelTip}}</button>
          <button @click.stop="submit"
            class="datepicker__button-select">{{this.local.submitTip}}</button>
        </div>
      </div>
    </transition>
  </div>
  ...
  methods: {
    ...
    handleBlur(){
        if (this.validateEvent) {
            this.dispatch('QwFormItem', 'qw.form.blur', this.text?this.get():'');
        }
    }
  },
  mounted() {
