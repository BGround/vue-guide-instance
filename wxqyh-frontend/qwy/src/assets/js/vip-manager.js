...
var isQwVip=function(isCallback,noCallback){
  var actionURL=baseURL+"/portal/vipPortalAction!isQwVip.action?requestSource=1";
  var isVip;
  $.ajax({
    url:actionURL,
    type:"post",
...
var isQwOverGoldVip=function(isCallback,noCallback){
  var actionURL=baseURL+"/portal/vipPortalAction!isOverGoldVIP.action?requestSource=1";
  var isOverGoldVip;
  $.ajax({
    url:actionURL,
    type:"post",
...
function _alertVip(functionName){
  if(isQuDao()){
    _.alert("提示","该功能仅限VIP会员使用");
    return;
  }
  if(!functionName){
    _.alert("提示","该功能仅限尊享版使用，<a href='https://tqy.do1.net.cn/qwy/qiweipublicity/companysrv/vip/vip_index.jsp' class='orange' target='_blank'>马上升级 ></a>");
    return;
  }
  _.alert("提示",functionName+"功能仅限尊享版使用，<a href='https://tqy.do1.net.cn/qwy/qiweipublicity/companysrv/vip/vip_index.jsp' class='orange' target='_blank'>马上升级 ></a>");
}
...
export {
  getSilverVipURL,isOldUserVip,hasVipPermission,isVipSilver,isVipGold,getPmsCode,canAccessInterface,isQuDao,showQw,_alertVip,getRegistDays,checkOpenShare
}
