...
const userConfigDirectory = path.join(os.homedir(),'/.webpack.custom.conf/qwy/');
...
let config = {
  ...
  dev: {
    ...
    url:'vm/module/approval.html',
    proxyTable: {
      '/wxqyh': {
        // target: 'http://qdtest.mychery.com:8080/',//代理路径
        target: 'http://v2j589.natappfree.cc/',//代理路径
        // secure:false,
        changeOrigin: true,
        pathRewrite: {
          // '^/wxqyh': '/'
...
function mergeConfig() {
  ...
  if(hasConfigFile || fs.existsSync(selfUseConfigFile)){
    try {
      let userConfig = hasConfigFile ? require(configFile) : require(selfUseConfigFile);
      ...
      console.log('已加载自定义配置文件：', hasConfigFile ? configFile : selfUseConfigFile);
    }catch (err) {
      console.error(err);
      return false;
    }
